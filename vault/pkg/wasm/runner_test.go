...
func TestExecute_ServLangWASM(t *testing.T) {
	...
	expected := "WASM_TRANSFORMED: hello Pranor wasm transform"
	if string(output) != expected {
		t.Errorf("expected %q, got %q", expected, string(output))
	}
}
...
func TestExecute_ServLangComplexWASM(t *testing.T) {
	wasmPath := "../../../Pranor/test_wasm_complex.wasm"
	wasmBytes, err := os.ReadFile(wasmPath)
	if err != nil {
		t.Skip("test_wasm_complex.wasm not compiled, compile first using pranor build")
	}

...
var SyslogEmitFunc = func(entry map[string]interface{}) {
	...
	if err != nil {
		log.Printf("[Pranor Chrono SYSLOG] failed to open log file %s: %v", logPath, err)
		return
	}
	defer f.Close()
...
func NewScheduler(acquireLockFunc func(jobID string, nextRun time.Time) bool) *Scheduler {
	endpoint := os.Getenv("PRANOR_STORE_ENDPOINT")
	bucket := os.Getenv("PRANOR_STORE_BUCKET")
	authToken := os.Getenv("PRANOR_STORE_AUTH_TOKEN")

	if endpoint == "" || authToken == "" {
		if raw := os.Getenv("PRANOR_DISCOVERY"); raw != "" {
			var manifest struct {
...
func (s *Scheduler) publishToQueue(topic, jobID string) {
	queueURL := os.Getenv("PRANOR_QUEUE_URL")
	if queueURL == "" {
		if raw := os.Getenv("PRANOR_DISCOVERY"); raw != "" {
			var manifest struct {
	...
	if err != nil {
		log.Printf("[JOB_CHAIN] Failed to create request to Pranor Pulse: %v", err)
		return
	}
	req.Header.Set("Content-Type", "application/json")

	authToken := os.Getenv("PRANOR_QUEUE_AUTH_TOKEN")
	if authToken == "" {
		if raw := os.Getenv("PRANOR_DISCOVERY"); raw != "" {
			var manifest struct {
				AuthToken string `json:"auth_token"`
	...
	resp, err := s.client.Do(req)
	if err != nil {
		log.Printf("[JOB_CHAIN] Failed to publish to Pranor Pulse topic %s: %v", topic, err)
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Printf("[JOB_CHAIN] Pranor Pulse returned status %d: %s", resp.StatusCode, string(body))
	} else {
		log.Printf("[JOB_CHAIN] Successfully triggered next job by publishing to topic %s on Pranor Pulse", topic)
	}
}

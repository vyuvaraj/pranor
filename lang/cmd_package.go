...
// addPackage generates a .pnr.d declaration file for a Go package.
func addPackage(pkgPath string) {
	...
	content.WriteString(fmt.Sprintf("// Generated by: pranor add %s\n\n", pkgPath))
	...
	declFile := filepath.Join(declDir, pkgName+".pnr.d")
	if err := os.WriteFile(declFile, []byte(content.String()), 0644); err != nil {
		fmt.Printf("Failed to write declaration file: %v\n", err)
		os.Exit(1)
	}

	// SL.H9: Update pranor.toml package resolution manifest
	if tomlBytes, err := os.ReadFile("pranor.toml"); err == nil {
		...
		if !strings.Contains(tomlContent, pkgPath) {
			...
			_ = os.WriteFile("pranor.toml", []byte(tomlContent), 0644)
		}
	}

	fmt.Printf("✓ Generated declaration: %s (%d functions)\n", declFile, len(functions))
	fmt.Printf("✓ Updated package manifest: pranor.toml (SL.H9)\n")
	fmt.Printf("\nUsage in your .pnr file:\n")
...
func listPackages() {
	...
	if err != nil {
		fmt.Println("No packages installed yet. Use: pranor add <package>")
		return
	}
	...
	for _, e := range entries {
		if !e.IsDir() && strings.HasSuffix(e.Name(), ".pnr.d") {
			name := strings.TrimSuffix(e.Name(), ".pnr.d")
...
func removePackage(name string) {
	declDir := "declarations"
	declFile := filepath.Join(declDir, name+".pnr.d")
	if _, err := os.Stat(declFile); os.IsNotExist(err) {
		fmt.Printf("Package '%s' not found in declarations/\n", name)
		fmt.Println("Use: pranor packages — to see installed packages")
		os.Exit(1)
	}
...
func getRegistryURL() string {
	url := os.Getenv("PRANOR_REGISTRY")
	if url == "" {
		url = "https://registry.Pranor.org"
	}
...
func tryGitFallback(name string, targetDir string, indent string) bool {
	...
	if strings.Contains(name, "/") {
		...
		gitURL = fmt.Sprintf("https://github.com/vyuvaraj/pranor-%s.git", name)
	}
...
// readPackageDependencies reads pranor.toml from an installed package directory
...
func readPackageDependencies(pkgDir string) []string {
	tomlPath := filepath.Join(pkgDir, "pranor.toml")
...
func publishPackage(pkgDir string) {
	...
	if jwtSecret := os.Getenv("PRANOR_JWT_SECRET"); jwtSecret != "" {
		token, err := generateJWT("pranor-client", []byte(jwtSecret))
		if err == nil {

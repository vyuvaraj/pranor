...
import (
	...
	"github.com/vyuvaraj/serv/packages/ServStore/pkg/analytics"
	"github.com/vyuvaraj/serv/packages/ServStore/pkg/erasure"
	"github.com/vyuvaraj/serv/packages/ServStore/pkg/opfs"
	"github.com/vyuvaraj/serv/packages/ServStore/pkg/s3wire"
)

func TestPhase48_StandaloneDaemonAndCLI(t *testing.T) {
	d, err := NewServStoreDaemon("")
	if err != nil {
...
func TestPhase48_BrowserStoreOPFSSync(t *testing.T) {
	...
	res, err := sdk.SyncToServStore(context.Background(), "/data.json")
	if err != nil || len(res) == 0 {
		t.Errorf("failed to sync OPFS file to ServStore: %v", err)
	}
}
...
func TestPhase48_ErasureCoding(t *testing.T) {
	...
	data := []byte("ServStore Enterprise Erasure Coding Data Chunk Test")

package import (
	...
	"github.com/vyuvaraj/pranor/notify/pkg/handlers"
	"github.com/vyuvaraj/pranor/notify/pkg/queue"
	"github.com/vyuvaraj/pranor/notify/pkg/storage"
)
...
func TestPranorNotifyTemplateAndChannels(t *testing.T) {
	setupTest()
...
func TestPranorNotifyRetriesAndDLQ(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mail/send", handleSend)
...
func TestPranorNotifyRateLimiting(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mail/send", handleSend)
...
func TestPranorNotifyTemplateVersioning(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mail/send", handleSend)
	...
	sendPayload1 := storage.SendRequest{
		...
		Version:  "v1",
		Context:  map[string]interface{}{"name": "Pranor"},
	}
	...
	if resS1.Body != "Welcome v1 to Pranor!" {
	...
	if resS2.Body != "Welcome v2 to Pranor! Enjoy your stay." {
		t.Errorf("expected rendered v2 content, got %q", resS2.Body)
	}
}

func TestPranorNotifyTrackingAndPreferences(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mail/send", handleSend)
...
func TestPranorNotifyDashboard(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/mail/send", handleSend)
...
func TestPranorNotifyAttachmentsColdTier(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()

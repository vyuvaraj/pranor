...
func main() {
	...
	if len(os.Args) > 1 && os.Args[0] != "inspect" {
		...
		case "inspect":
			...
			os.Exit(0)
		case "version":
			fmt.Printf("Pranor Mesh v%s\n", version)
			os.Exit(0)
		case "help":
			fmt.Printf("Pranor Mesh v%s\n\n", version)
			fmt.Println("Subcommands:")
			fmt.Println("  (default)  Start the Pranor Mesh registry daemon")
	...
	if *verFlag {
		fmt.Printf("Pranor Mesh Registry v%s\n", version)
		return
	}

	log.Printf("Starting Pranor Mesh Registry v%s on port :%d...", version, *port)
	...
	log.Println("Shutting down Pranor Mesh Registry gracefully...")
	shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	r.Close()
	log.Println("Pranor Mesh Registry shutdown complete.")
}
...
func startServer(srv *http.Server) {
	if os.Getenv("SERVMESH_MTLS_REQUIRED") == "true" {
		...
		if certFile != "" && keyFile != "" {
			...
			log.Printf("[Pranor Mesh] Enforcing mutual TLS on address %s", srv.Addr)
			go func() {

...
import (
	...
	"github.com/vyuvaraj/pranor/core/pkg/middleware"
	"github.com/vyuvaraj/pranor/auth/pkg/handlers"
	"github.com/vyuvaraj/pranor/auth/pkg/kms"
	"github.com/vyuvaraj/pranor/auth/pkg/mfa"
	"github.com/vyuvaraj/pranor/auth/pkg/sessions"
	"github.com/vyuvaraj/pranor/auth/pkg/store"
)
...
func TestPranorAuthWorkflow(t *testing.T) {
	setupTest()
...
func TestPranorAuthOAuthToken(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
...
func TestPranorAuthSecurityLockoutAndReset(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/auth/register", handlers.HandleRegister)
	mux.HandleFunc("/api/auth/login", handlers.HandleLogin)
...
func TestPranorAuthKeysAndSessions(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/auth/register", handlers.HandleRegister)
	mux.HandleFunc("/api/auth/login", handlers.HandleLogin)
...
func (m *mockMfaEngine) Setup(username string) (string, string, error) {
	...
	qrCodeURL := "https://api.qrserver.com/v1/create-qr-code/?data=otpauth://totp/Pranor:" + username
...
func TestPranorAuthTenancyAndMfa(t *testing.T) {
	setupTest()
...
func (m *mockSocialProvider) Redirect(w http.ResponseWriter, r *http.Request, provider string) {
	...
	_ = core.EmitAuditEvent("pranor-auth", "SOCIAL_LOGIN_REDIRECT", "guest", map[string]interface{}{"provider": provider})
...
func TestPranorAuthSocialLogin(t *testing.T) {
	setupTest()
...
func TestPranorAuthUserMgmtAndSecrets(t *testing.T) {
	setupTest()
	mux := http.NewServeMux()
	mux.HandleFunc("/api/auth/register", handlers.HandleRegister)
	mux.HandleFunc("/api/auth/login", handlers.HandleLogin)
...
func TestPranorAuthSecurityFeatures(t *testing.T) {
	setupTest()
...
func TestTraceparentPropagation(t *testing.T) {
	...
	handler := core.TraceMiddleware("pranor-auth", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {

...
func TestHotReload(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "PRANOR_hot_reload_test")
	if err != nil {
...
`
	...
	// Build the pranor.exe binary to make sure it includes our latest changes
	goPath, err := resolveGoPath()
	if err != nil {
		t.Fatalf("failed to find Go compiler: %v", err)
	}

	servBin := filepath.Join(tmpDir, "PRANOR_test_runner.exe")
	buildCmd := exec.Command(goPath, "build", "-o", servBin, ".")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("failed to compile pranor compiler binary: %v", err)
	}

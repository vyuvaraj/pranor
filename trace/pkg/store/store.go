...
func NewStore(limit int) *Store {
	samplingRate := 50
	if env := os.Getenv("PRANOR_TRACE_SAMPLING_RATE"); env != "" {
		if val, err := strconv.Atoi(env); err == nil {
			samplingRate = val
		}
	}
	adaptiveErrorThreshold := 0.05
	if env := os.Getenv("PRANOR_TRACE_ADAPTIVE_ERROR_THRESHOLD"); env != "" {
...
func (s *Store) getServiceSamplingRate(service string) int {
	...
	envKey := "PRANOR_TRACE_SAMPLING_RATE_" + normalized
...
func (s *Store) getServiceTTL(service string) time.Duration {
	...
	if val := os.Getenv("PRANOR_TRACE_DEFAULT_TTL"); val != "" {
	...
	normalized := strings.ToUpper(strings.ReplaceAll(strings.ReplaceAll(service, "-", "_"), ".", "_"))
	envKey := "PRANOR_TRACE_TTL_" + normalized
	if val := os.Getenv(envKey); val != "" {
		if d, err := time.ParseDuration(val); err == nil {
			return d
		}

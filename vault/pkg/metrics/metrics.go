...
func Handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprintln(w, "# HELP pranorVault_http_requests_total Total number of HTTP requests processed.")
		fmt.Fprintln(w, "# TYPE pranorVault_http_requests_total counter")
		for key, count := range registry.httpRequests {
			...
			fmt.Fprintf(w, "pranorVault_http_requests_total{method=\"%s\",path=\"%s\",status=\"%s\"} %d\n", method, path, status, count)
		}

		// Write http_in_flight_requests
		fmt.Fprintln(w, "\n# HELP pranorVault_http_in_flight_requests Current number of HTTP requests being processed.")
		fmt.Fprintln(w, "# TYPE pranorVault_http_in_flight_requests gauge")
		fmt.Fprintf(w, "pranorVault_http_in_flight_requests %d\n", registry.inFlightRequests)

		// Write request durations
		fmt.Fprintln(w, "\n# HELP pranorVault_http_request_duration_seconds HTTP request latencies in seconds.")
		fmt.Fprintln(w, "# TYPE pranorVault_http_request_duration_seconds summary")
		for key, sum := range registry.requestDuration {
			parts := splitKey(key)
			if len(parts) == 2 {
				method, path := parts[0], parts[1]
				count := registry.requestCount[key]
				fmt.Fprintf(w, "pranorVault_http_request_duration_seconds_sum{method=\"%s\",path=\"%s\"} %f\n", method, path, sum)
				fmt.Fprintf(w, "pranorVault_http_request_duration_seconds_count{method=\"%s\",path=\"%s\"} %d\n", method, path, count)
			}
		}

		// Write S3 latency and throughput performance metrics
		s3Mu.Lock()
		fmt.Fprintln(w, "\n# HELP pranorVault_s3_upload_bytes_total Total number of bytes uploaded to S3.")
		fmt.Fprintln(w, "# TYPE pranorVault_s3_upload_bytes_total counter")
		fmt.Fprintf(w, "pranorVault_s3_upload_bytes_total %d\n", s3UploadBytes)

		fmt.Fprintln(w, "\n# HELP pranorVault_s3_download_bytes_total Total number of bytes downloaded from S3.")
		fmt.Fprintln(w, "# TYPE pranorVault_s3_download_bytes_total counter")
		fmt.Fprintf(w, "pranorVault_s3_download_bytes_total %d\n", s3DownloadBytes)

		fmt.Fprintln(w, "\n# HELP pranorVault_s3_operations_total Total number of S3 actions executed.")
		fmt.Fprintln(w, "# TYPE pranorVault_s3_operations_total counter")
		for key, count := range s3Operations {
			parts := splitKey(key)
			if len(parts) == 2 {
				fmt.Fprintf(w, "pranorVault_s3_operations_total{op=\"%s\",status=\"%s\"} %d\n", parts[0], parts[1], count)
			}
		}
		s3Mu.Unlock()
	})

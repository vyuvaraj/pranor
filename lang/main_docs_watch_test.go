...
func TestDocsServeAndWatch(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "PRANOR_docs_serve_test")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tmpDir)

	fileArg := filepath.Join(tmpDir, "app.pnr")
	initialContent := `
route "GET" "/hello" (req) {
	return "hello"
}
`
	if err := os.WriteFile(fileArg, []byte(initialContent), 0644); err != nil {
		t.Fatalf("failed to write app.pnr: %v", err)
	}
...
`
	if err := os.WriteFile(fileArg, []byte(updatedContent), 0644); err != nil {
		t.Fatalf("failed to update app.pnr: %v", err)
	}

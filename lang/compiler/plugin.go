...
// Plugins are compiled as Go plugins (.so/.dll) from .pnr.plugin.go files.
...
func LoadAndRunPlugins(program *Program, workspaceDir string) []Diagnostic {
	...
	pattern := filepath.Join(workspaceDir, "*.pnr.plugin.so")
	if os.PathSeparator == '\\' {
		pattern = filepath.Join(workspaceDir, "*.pnr.plugin.dll")
	}

...
var servDevServices = []struct {
	...
	{"github.com/vyuvaraj/pranor/vault", "8081", "github.com/vyuvaraj/pranor/vault", []string{"--port", "8081", "--data-dir", ".servdev/store-data"}, nil},
	{"github.com/vyuvaraj/pranor/pulse", "8082", "github.com/vyuvaraj/pranor/pulse", nil, nil},
	{"github.com/vyuvaraj/pranor/cache", "8086", "github.com/vyuvaraj/pranor/cache", nil, nil},
	{"github.com/vyuvaraj/pranor/chrono", "8087", "github.com/vyuvaraj/pranor/chrono", nil, []string{"PORT=8087"}},
	{"github.com/vyuvaraj/pranor/gate", "8080", "github.com/vyuvaraj/pranor/gate", nil, nil},
	{"github.com/vyuvaraj/pranor/trace", "8090", "github.com/vyuvaraj/pranor/trace", nil, nil},
	{"github.com/vyuvaraj/pranor/auth", "8091", "github.com/vyuvaraj/pranor/auth", nil, nil},
	{"ServDB", "8092", "servdb", nil, nil},
	{"github.com/vyuvaraj/pranor/notify", "8093", "github.com/vyuvaraj/pranor/notify", nil, nil},
	{"github.com/vyuvaraj/pranor/flow", "8094", "github.com/vyuvaraj/pranor/flow", nil, nil},
}
...
func runDevCmd() {
	...
	portFlag := devCmd.String("port", "8080", "Port for the user's .pnr service")
	noConsoleFlag := devCmd.Bool("no-console", false, "Skip opening Pranor Console dashboard")
	...
	fmt.Println()
	fmt.Println("  ▲ pranor Dev Environment")
	fmt.Println("  ━━━━━━━━━━━━━━━━━━━━━━")
	fmt.Println()
	...
	for _, s := range detectRequiredServices(srvFile) {
		...
		case "mail":
			requestedServices["github.com/vyuvaraj/pranor/notify"] = true
		case "flow":
			requestedServices["github.com/vyuvaraj/pranor/flow"] = true
		}
	}
	...
	os.Setenv("PRANOR_STORE_ENDPOINT", "http://localhost:8081")
	os.Setenv("PRANOR_QUEUE_ENDPOINT", "localhost:8082")
	os.Setenv("PRANOR_CACHE_ENDPOINT", "http://localhost:8086")
	os.Setenv("PRANOR_GATE_ENDPOINT", "http://localhost:8080")
	os.Setenv("PRANOR_TRACE_ENDPOINT", "http://localhost:8090")
	os.Setenv("PRANOR_AUTH_ENDPOINT", "http://localhost:8091")
	os.Setenv("PRANOR_DB_ENDPOINT", "http://localhost:8092")
	os.Setenv("PRANOR_MAIL_ENDPOINT", "http://localhost:8093")
	os.Setenv("PRANOR_FLOW_ENDPOINT", "http://localhost:8094")

	if !*noConsoleFlag {
		fmt.Println("  Dashboard: http://localhost:8083 (start Pranor Console separately)")
	}
...
func parseServiceList(input string) map[string]bool {
	...
	for _, s := range splitComma(input) {
		switch s {
		case "store":
			result["github.com/vyuvaraj/pranor/vault"] = true
		case "queue":
			result["github.com/vyuvaraj/pranor/pulse"] = true
		case "cache":
			result["github.com/vyuvaraj/pranor/cache"] = true
		case "cron":
			result["github.com/vyuvaraj/pranor/chrono"] = true
		case "gate":
			result["github.com/vyuvaraj/pranor/gate"] = true
		case "trace":
			result["github.com/vyuvaraj/pranor/trace"] = true
		case "auth":
			result["github.com/vyuvaraj/pranor/auth"] = true
		case "db":
			result["ServDB"] = true
		case "mail":
			result["github.com/vyuvaraj/pranor/notify"] = true
		case "flow":
			result["github.com/vyuvaraj/pranor/flow"] = true
		}
	}
	return result
}
					...
					if exists {
						...
						if now.Sub(lastTrigger) > triggerDelay {
							...
							if err := buildCmd.Run(); err != nil {
								log.Printf("[WORKSPACE HOT-RELOAD] Build failed for %s: %v. Keeping previous running process serving traffic.", proc.name, err)
								continue
							}
...
// implementing the DX.26 k9s-style terminal dashboard for 'pranor dev --dashboard'.
func startDevDashboard(procs []*devProcess) {
	...
	for range ticker.C {
		...
		sb.WriteString("┌─────────────────────────────────────────────────────────────┐\n")
		sb.WriteString(fmt.Sprintf("│  ▲ pranor Dev Dashboard                         %s  │\n", timestamp))
		sb.WriteString("├──────────────────┬──────────┬──────────┬────────────────────┤\n")

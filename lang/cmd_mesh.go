...
// runMeshInspect prints the live state of the Pranor Mesh service registry.
// Usage: pranor mesh inspect [--host <url>] [--service <name>]
func runMeshInspect() {
	host := "http://localhost:8083"
	service := ""

	if envHost := os.Getenv("PRANOR_MESH_URL"); envHost != "" {
		host = envHost
	}
	...
	if err != nil {
		fmt.Printf("Error connecting to Pranor Mesh at %s: %v\n", host, err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		fmt.Printf("Pranor Mesh returned %d: %s\n", resp.StatusCode, string(body))
		os.Exit(1)
	}
	...
	if len(instances) == 0 {
		if service != "" {
			...
			fmt.Println("No services registered in Pranor Mesh.")
		}
		return
	}

	fmt.Printf("Pranor Mesh Registry  (%d instances)\n", len(instances))
...
// Usage: pranor mesh routes [--host <url>]
func runMeshRoutes() {
	host := "http://localhost:8083"
	if envHost := os.Getenv("PRANOR_MESH_URL"); envHost != "" {
		host = envHost

...
// setBreakpoints requests and stackTrace responses, translating .pnr
...
// them to dlv, intercepting setBreakpoints to translate .pnr → .go lines.
...
// (stdout), intercepting stackTrace responses to translate .go → .pnr lines.
...
// .pnr file paths → .go paths and .pnr line numbers → .go line numbers.
func (p *Proxy) translateClientMessage(raw []byte) ([]byte, error) {
	...
	if !isSrvFile(args.Source.Path, p.pnrFile) {
		return raw, nil
	}
	...
	for i, srvLine := range args.Lines {
		if goLine, ok := p.sm.pnrToGo(srvLine); ok {
			args.Lines[i] = goLine
		}
	}
...
// .go paths → .pnr paths and .go line numbers → .pnr line numbers.

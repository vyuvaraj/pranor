...
func TestPrepareRename(t *testing.T) {
	...
	msg := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":0,"character":5}}`),
	}
	...
	msg2 := JSONRPCMessage{
		...
		ID:      2,
		Method:  "textDocument/prepareRename",
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":2,"character":1}}`),
	}
...
fn other_func() {
	...
	msg := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":1,"character":5}}`),
	}
	...
	msgRename := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":1,"character":5},"newName":"newVal"}`),
	}
...
func TestReferencesAndRenameGlobalScope(t *testing.T) {
	...
	server.documents[uri2] = `import "file1.pnr"
fn main() {
	let x = global_helper()
}`
	// Add global symbols
	...
	// References on "global_helper" (line 0, col 3 in file1.pnr)
	msg := JSONRPCMessage{
		JSONRPC: "2.0",
		ID:      1,
		Method:  "textDocument/references",
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///file1.pnr"},"position":{"line":0,"character":3}}`),
	}
	...
	// Should find global_helper definition in file1.pnr and usage in file2.pnr
	if len(locations) != 2 {
	...
	msgRename := JSONRPCMessage{
		...
		ID:      2,
		Method:  "textDocument/rename",
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///file1.pnr"},"position":{"line":0,"character":3},"newName":"helper_new"}`),
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			msg := JSONRPCMessage{
				...
				ID:      1,
				Method:  "textDocument/completion",
				Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":0,"character":` + string(rune('0'+tc.charPos/10)) + string(rune('0'+tc.charPos%10)) + `}}`),
			}
...
func TestStructFieldCompletionsDX5(t *testing.T) {
	...
	openMsg := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr","languageId":"github.com/vyuvaraj/pranor/lang","version":1,"text":"struct User {\n  id: string\n  name: string\n}\nlet u = User {\n  id: \"1\"\n  name: \"Alice\"\n}\nlet x = u."}}`),
	}
...
func TestImportPathCompletionsDX4(t *testing.T) {
	...
	uri := "file:///f:/Don/Pranor/Pranor/lsp/test_main.pnr"
	...
	// Result could be an empty list (no .pnr files in that dir) or a list — just assert no crash
	if resp.Result == nil {
...
func TestCodeLensDX12RouteArgs(t *testing.T) {
	server := NewServer()
	uri := "file:///test.pnr"
	// Pranor route syntax: route "METHOD" "/path" (param) { ... }
	...
	if !strings.Contains(body, "file:///test.pnr") {
...
func TestServURLNavigationDX16(t *testing.T) {
	server := NewServer()
	// Use a real temp path structure: workspace/svc-a/main.pnr -> pranor://svc-b
	// We can't guarantee svc-b/main.pnr exists on disk, so just verify no crash + nil
	uri := "file:///f:/Don/Pranor/Pranor/main.pnr"
	server.documents[uri] = `let url = "pranor://auth/login"`
	paramsJSON, _ := json.Marshal(map[string]interface{}{
		"textDocument": map[string]string{"uri": uri},
...
func TestAICompletionDisabledDX14(t *testing.T) {
	...
	if items != nil {
		t.Errorf("DX.14: expected nil when PRANOR_AI_COMPLETION_ENDPOINT not set, got %v", items)
	}
}
...
func TestHover(t *testing.T) {
	...
	msg1 := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":0,"character":6}}`),
	}
	...
	msg2 := JSONRPCMessage{
		...
		ID:      2,
		Method:  "textDocument/hover",
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":0,"character":1}}`),
	}
...
fn main() {
	...
	msg := JSONRPCMessage{
		...
		Params:  json.RawMessage(`{"textDocument":{"uri":"file:///test.pnr"},"position":{"line":2,"character":2}}`),
	}
...
func TestDefinitionCrossFileImport(t *testing.T) {
	// Create two temporary .pnr files representing imported module and importing module
	tmpDir := t.TempDir()
	importedFile := filepath.Join(tmpDir, "imported.pnr")
	importingFile := filepath.Join(tmpDir, "main.pnr")
	...
	importingContent := "import \"imported.pnr\"\nfn start() {\n\texternalHelper()\n}"

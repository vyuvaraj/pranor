...
import (
	...
	"github.com/vyuvaraj/pranor/flow/pkg/handlers"
...
func initStore(dbDriver, dbURL string) {
	if dbURL != "" {
		...
		workflowStore = store
		log.Printf("[INFO] Pranor Flow using SQL database storage backend: driver=%s", drv)
	} else {
		client := Pranor Core.NewStoreClient()
		workflowStore = storage.NewPranorVaultWorkflowStore(client)
	}
	loadStateFromStore()
}
...
func main() {
	portStr := flag.String("port", "8096", "Pranor Flow server port")
	...
	if standalone {
		log.Println("[INFO] Pranor Flow: Running in standalone mode. Store persistence redirected to local directory.")
	}
	...
	mux.HandleFunc("/api/version", Pranor Core.VersionHandler("github.com/vyuvaraj/pranor/flow", "1.0.0"))
	mux.HandleFunc("/api/v1/version", Pranor Core.VersionHandler("github.com/vyuvaraj/pranor/flow", "1.0.0"))
	...
	serverHandler := Pranor Core.TraceMiddleware("github.com/vyuvaraj/pranor/flow",
		rateLimiter(
	...
	go func() {
		log.Printf("[INFO] Pranor Flow engine starting on port %s", port)
		if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("failed to start Pranor Flow: %v", err)
		}
	}()

	<-stop

	log.Println("[INFO] Shutting down Pranor Flow server...")
	...
	log.Println("[INFO] Pranor Flow server exited cleanly")
}

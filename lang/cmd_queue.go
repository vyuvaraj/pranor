...
// runQueueTail streams recent messages from a Pranor Pulse topic to stdout.
// Usage: pranor queue tail <topic> [--host <url>] [--limit <n>]
func runQueueTail() {
	if len(os.Args) < 4 {
		fmt.Println("Usage: pranor queue tail <topic> [--host <url>] [--limit <n>]")
		fmt.Println("       pranor queue tail <topic> --host http://localhost:8085")
		os.Exit(1)
	}
	...
	if envHost := os.Getenv("PRANOR_QUEUE_URL"); envHost != "" {
		host = envHost
	}
	...
	if token := os.Getenv("PRANOR_TOKEN"); token != "" {
	...
	if err != nil {
		fmt.Printf("Error connecting to Pranor Pulse at %s: %v\n", host, err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		fmt.Printf("Pranor Pulse returned %d: %s\n", resp.StatusCode, string(body))
		os.Exit(1)
	}
...
// runQueueList lists all topics registered in Pranor Pulse.
// Usage: pranor queue list [--host <url>]
func runQueueList() {
	host := "http://localhost:8085"
	if envHost := os.Getenv("PRANOR_QUEUE_URL"); envHost != "" {
		host = envHost
...
// Usage: pranor queue dlq inspect <topic> [--host <url>] [--replay]
func runQueueDLQ() {
	if len(os.Args) < 5 || os.Args[3] != "inspect" {
		fmt.Println("Usage: pranor queue dlq inspect <topic> [--host <url>] [--replay]")
		os.Exit(1)
	}

...
func (tt *TelemetryTracker) StartTaskSpan(instanceID, taskName, parentTraceparent string) (*ServShared.Span, string) {
	...
	span := ServShared.StartSpan(fmt.Sprintf("servflow:task %s %s", instanceID, taskName), parentTraceparent)
	if span != nil {
...
func (tt *TelemetryTracker) EndTaskSpan(span *ServShared.Span, instanceID, taskName string, err error) StepCostRecord {
	...
	if span != nil {
		...
		ServShared.EndSpan(span, err, map[string]interface{}{
			"servflow.instance_id": instanceID,
			"servflow.task_name":   taskName,
		})

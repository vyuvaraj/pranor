...
import (
	...
	"github.com/vyuvaraj/serv/packages/ServQueue/pkg/opfs"
	"github.com/vyuvaraj/serv/packages/ServQueue/pkg/relay"
)
...
func main() {
	...
	// Register JS global functions for ServQueue Embedded
	js.Global().Set("ServQueueEnqueue", js.FuncOf(jsEnqueue))
	js.Global().Set("ServQueueDequeue", js.FuncOf(jsDequeue))
	js.Global().Set("ServQueueSyncOutbox", js.FuncOf(jsSyncOutbox))
	js.Global().Set("ServQueueGetPendingSync", js.FuncOf(jsGetPendingSync))
...
func jsEnqueue(this js.Value, args []js.Value) interface{} {
	if len(args) < 2 {
		return js.ValueOf(map[string]interface{}{"error": "Usage: ServQueueEnqueue(topic, payload)"})
	}
	topic := args[0].String()
...
func jsDequeue(this js.Value, args []js.Value) interface{} {
	if len(args) < 3 {
		return js.ValueOf(map[string]interface{}{"error": "Usage: ServQueueDequeue(topic, startOffset, limit)"})
	}
	topic := args[0].String()

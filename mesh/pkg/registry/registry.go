...
func (r *Registry) generateRootCA() {
	...
	template := &x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName:   "Pranor Mesh Root CA",
...
func (r *Registry) Handler() http.Handler {
	...
	mux.HandleFunc("/api/csr", func(w http.ResponseWriter, req *http.Request) {
		...
		template := &x509.Certificate{
			SerialNumber: serialNumber,
			Subject: pkix.Name{
				CommonName:   body.Service + ".Pranor",
				Organization: []string{"Pranor"},
			},
			DNSNames:    []string{body.Service, body.Service + ".Pranor", "localhost"},
			NotBefore:   time.Now().Add(-1 * time.Hour),
	...
	mux.HandleFunc("/api/mesh/spiffe/svid", func(w http.ResponseWriter, req *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			"spiffe_id":     fmt.Sprintf("spiffe://Pranor Mesh.domain/ns/default/sa/%s", svc),
			"svid_type":     "X509-SVID",

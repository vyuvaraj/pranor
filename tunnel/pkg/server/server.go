// Package server implements the Pranor Tunnel relay server.
//
...
import (
	...
	"github.com/vyuvaraj/pranor/tunnel/pkg/tunnel"
...
// Server is the Pranor Tunnel relay server.
type Server struct {
	addr          string
	baseDomain    string // e.g., "Pranor.net" or "localhost"
...
func (s *Server) Start() error {
	...
	if autocertEnabled && autocertDomain != "" {
		...
		log.Printf("Pranor Tunnel relay listening with Auto-TLS (Let's Encrypt) on %s (base domain: %s)", s.addr, s.baseDomain)
		return s.httpSrv.ListenAndServeTLS("", "")
	}

	if tlsCert != "" && tlsKey != "" {
		log.Printf("Pranor Tunnel relay listening with TLS on %s (base domain: %s)", s.addr, s.baseDomain)
		return s.httpSrv.ListenAndServeTLS(tlsCert, tlsKey)
	}

	log.Printf("Pranor Tunnel relay listening on %s (base domain: %s)", s.addr, s.baseDomain)
	...
	if !exists {
		...
		if subdomain == "" {
			...
			json.NewEncoder(w).Encode(map[string]string{
				"service": "github.com/vyuvaraj/pranor/tunnel",
				"status":  "running",
				"hint":    "Connect via: Pranor Tunnel client <port> --relay ws://" + r.Host + "/ws/connect",
			})
			return
		}
	...
	if tc.sharingAuth != "" {
		...
		if !inviteValid {
			...
			if !ok || username != expectedUser || password != expectedPass {
				...
				w.Header().Set("WWW-Authenticate", `Basic realm="Pranor Tunnel Share"`)
...
// For "abc123.Pranor.net:8443" with baseDomain "Pranor.net", returns "abc123".

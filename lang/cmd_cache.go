...
func runCacheInspect() {
	...
	if envHost := os.Getenv("PRANOR_CACHE_URL"); envHost != "" {
		host = envHost
	}
	...
	authToken := os.Getenv("PRANOR_CACHE_AUTH_TOKEN")
	if authToken == "" {
	...
	if err != nil {
		fmt.Printf("Failed to connect to Pranor Cache: %v\n", err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Printf("Pranor Cache returned error status %d: %s\n", resp.StatusCode, string(body))
		os.Exit(1)
	}
	...
	fmt.Println("=== Pranor Cache Telemetry Inspector ===")

...
2. Investigate the codebase at `/home/developer/workspace/serv/packages/ServAuth` and `/home/developer/workspace/serv/packages/ServCache`.
...
4. Detail requirements for:
   - SA.G1: Opaque session token store (`packages/ServAuth/pkg/sessions/token_store.go`)
   - SA.G6: Credential stuffing velocity limiter (`packages/ServAuth/pkg/security/velocity_limiter.go`)
   - SC.G3: Probabilistic Bloom filter (`packages/ServCache/pkg/bloom/bloom.go`)
   - SC.G4: Tiered TTL policy engine (`packages/ServCache/pkg/tieredttl/policy.go`)

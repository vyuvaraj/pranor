...
import (
	...
	"github.com/vyuvaraj/pranor/mesh/pkg/pb"
...
func (t *MeshTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		startTime := time.Now()
		if os.Getenv("PRANOR_MESH_GRPC") == "true" {
...
func (t *MeshTransport) renewCertificate(ctx context.Context, serviceName string, jwtToken string) (tls.Certificate, []byte, error) {
	...
	subj := pkix.Name{
		CommonName:   serviceName + ".Pranor",
		Organization: []string{"Pranor"},
	}
	template := &x509.CertificateRequest{
		Subject:            subj,
		SignatureAlgorithm: x509.ECDSAWithSHA256,
		DNSNames:           []string{serviceName, serviceName + ".Pranor", "localhost"},
	}

...
// runPluginCmd handles the `serv plugin` subcommand.
func runPluginCmd() {
	if len(os.Args) < 3 {
		printPluginUsage()
		return
	...
	case "pull":
		...
		args := pullCmd.Args()
		if len(args) < 1 {
			fmt.Println("Usage: serv plugin pull <plugin-name> [-o <path>] [--version <v0.1.0>]")
			fmt.Println("Example: serv plugin pull jwt-auth")
			fmt.Println("         serv plugin pull pii-scrubber -o ./custom/plugins/")
			os.Exit(1)
		}
...
func printPluginUsage() {
	fmt.Println("Serv WASM Plugin Manager (`vyuvaraj/serv-wasm-plugins` repository)")
	fmt.Println("Usage:")
	fmt.Println("  serv plugin pull <plugin-name> [-o <path>]   Pull pre-compiled WASM plugin binary")
	fmt.Println("  serv plugin list                              List available and installed plugins")
	fmt.Println("  serv plugin search <query>                   Search community plugin repository")
}
...
func pullPlugin(pluginName, customOutPath, version string) {
	...
	fmt.Printf("📦 Pulling WASM plugin '%s' (%s) from vyuvaraj/serv-wasm-plugins...\n", cleanName, version)

	// Primary download URL from GitHub Releases
	downloadURLs := []string{
		fmt.Sprintf("https://github.com/vyuvaraj/serv-wasm-plugins/releases/download/%s/%s", version, wasmFileName),
		fmt.Sprintf("https://raw.githubusercontent.com/vyuvaraj/serv-wasm-plugins/main/plugins/%s", wasmFileName),
	}
	...
	fmt.Printf("   Target Component: ServGate / ServQueue / ServStore WASM Runner\n")
	fmt.Printf("\nUsage in ServGate config or Serv-lang:\n")
...
func listPlugins() {
	fmt.Println("🌐 Community WASM Plugins (`vyuvaraj/serv-wasm-plugins` v0.1.0):")
	...
	if err != nil || len(files) == 0 {
		fmt.Println("   (No local plugins installed yet. Run `serv plugin pull <name>` to install)")
		return
	}
...
func searchPlugins(query string) {
	fmt.Printf("🔍 Searching community repository `vyuvaraj/serv-wasm-plugins` for '%s':\n\n", query)
...
func generateWasmStub(name string) []byte {
	...
	// Custom section: name = "serv-plugin-meta"
	meta := fmt.Sprintf("serv-plugin:%s:v0.1.0:vyuvaraj/serv-wasm-plugins", name)

# Changes Summary — Milestone M2 (ServCache)

## Created Files

1. **`packages/ServCache/pkg/bloom/bloom.go`**
...
2. **`packages/ServCache/pkg/bloom/bloom_test.go`**
...
3. **`packages/ServCache/pkg/tieredttl/policy.go`**
...
4. **`packages/ServCache/pkg/tieredttl/policy_test.go`**
...
- Run `git diff go.mod` in `packages/ServCache`: no changes. Zero external dependencies added.

package import (
	...
	mailtemplate "github.com/vyuvaraj/pranor/notify/pkg/template"
)
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if deliveryErr == nil {
			...
			case "email":
				...
				if dkimSig != "" {
					log.Printf("[Pranor Notify] [DKIM] Added signature: %s", dkimSig)
				}
				log.Printf("[Pranor Notify] [EMAIL] Sending to %s: %s", req.Target, bodyStr)
				ctx.MockedEmailsMu.Lock()
				*ctx.MockedEmails = append(*ctx.MockedEmails, storage.MockEmail{
					From:    "http-api@Pranor Notify",
					To:      req.Target,
			...
			case "slack":
				log.Printf("[Pranor Notify] [SLACK] Posting to webhook %s: %s", req.Target, bodyStr)
			case "sms":
		...
		log.Printf("[Pranor Notify] Attempt %d failed: %v. Retrying in %v...", attempt, deliveryErr, backoff)
		sleepFn := ctx.SleepFn
		if sleepFn == nil {
			sleepFn = time.Sleep
		}

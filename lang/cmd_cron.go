...
func runCronList() {
	...
	if envHost := os.Getenv("PRANOR_CRON_URL"); envHost != "" {
		host = envHost
	}
	...
	authToken := os.Getenv("PRANOR_CRON_AUTH_TOKEN")
	if authToken == "" {
	...
	if err != nil {
		fmt.Printf("Failed to connect to Pranor Chrono at %s: %v\n", host, err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Printf("Pranor Chrono returned status %d: %s\n", resp.StatusCode, string(body))
		os.Exit(1)
	}
	...
	fmt.Println("=== Pranor Chrono Scheduled Jobs ===")
	if len(jobs) == 0 {

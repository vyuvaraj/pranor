...
func TestPanicStackTraceMapping(t *testing.T) {
	// 1. Create a temporary .pnr file that triggers a division-by-zero panic
	tmpFile, err := os.CreateTemp("", "test_panic_*.pnr")
	if err != nil {
...
`
	...
	servBin := filepath.Join(os.TempDir(), fmt.Sprintf("PRANOR_test_compiler_%d.exe", time.Now().UnixNano()))
	...
	// 4. Run the compiled pranor binary with 'test' subcommand to execute the panicking test
	...
	// 5. Verify that the output stack trace was rewritten to point to the .pnr file and correct line
	...
	// Also make sure we mapped back to a line in the .pnr file, e.g. line 3 or 7

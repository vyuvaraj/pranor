...
func TestORMIntegration(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_orm_*.pnr")
	if err != nil {
...
extern fn runMigrations() from "go:github.com/vyuvaraj/pranor/lang/runtime:RunMigrations"
...
func TestMySQLDatabaseSchemeParser(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_mysql_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
...
func TestSearchKeywordAndAdapter(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_search_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
...
import { query, index } from "stdlib/search.pnr"

test "search adapter integration test" {
	let doc1 = {"id": "1", "title": "Pranor architecture guide", "category": "tech"}
...
func TestCsvAndStringPadHelpers(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_csv_pad_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
	...
	srvContent := `
import { parseCSV, toCSV, parseRow, toRow } from "stdlib/csv.pnr"
import { padLeft, padRight } from "stdlib/strings_util.pnr"
...
func TestTursoJwtAndUrlHelpers(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_turso_jwt_url_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
...
import { jwtEncode, jwtIsExpired } from "stdlib/jwt.pnr"
import { parseQuery, buildQuery, extractPath } from "stdlib/url.pnr"
...
test "Turso, JWT expiry, and URL helpers test" {
	...
	let pth = extractPath("https://Pranor.io/docs/intro")

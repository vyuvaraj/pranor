...
type SourceMap struct {
	// srvFile is the absolute path of the original .pnr source file.
	srvFile string
	// goFile is the absolute path of the generated Go file.
	goFile string

	// goToSrv maps a Go line number → .pnr line number.
	goToSrv map[int]int
	// srvToGo maps a .pnr line number → the first Go line that represents it.
	srvToGo map[int]int
...
// GoToSrv returns the .pnr line number that corresponds to the given Go line
...
// GoToSrvApprox returns the .pnr line number for the given Go line, or if no

...
func main() {
	...
	if *verFlag {
		fmt.Printf("Pranor Trace Collector v%s\n", version)
		return
	}

	log.Printf("Starting Pranor Trace Collector v%s...", version)
	...
	go func() {
		log.Printf("Pranor Trace OTLP/HTTP listening on http://localhost:%d", *port)
	...
	log.Println("Shutting down Pranor Trace Collector gracefully...")
	...
	log.Println("Pranor Trace Collector shutdown complete.")
}

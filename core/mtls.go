...
// from the Pranor Mesh CA. Use this for all inter-service communication in production.
...
// provisioned from the Pranor Mesh CA. If PRANOR_MESH_URL is not set, returns
// a plain transport (dev mode).
func SecureTransport(serviceName string) *http.Transport {
	meshURL := os.Getenv("PRANOR_MESH_URL")
	if meshURL == "" {
...
func (mt *MeshTransport) provisionCert() (*http.Transport, error) {
	...
	csrTemplate := &x509.CertificateRequest{
		Subject: pkix.Name{
			CommonName:   mt.serviceName + ".Pranor",
			Organization: []string{"Pranor"},
		},
		DNSNames: []string{mt.serviceName, mt.serviceName + ".Pranor", "localhost"},
	}

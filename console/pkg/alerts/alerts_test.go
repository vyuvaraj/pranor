...
func TestAlertsAddingUpdatingClearing(t *testing.T) {
	...
	// 1. Add alert
	AddOrUpdateAlert("github.com/vyuvaraj/pranor/gate", "offline", "Pranor Gate offline msg", "critical")
	if len(alertsList) != 1 {
		t.Fatalf("expected 1 alert, got %d", len(alertsList))
	}
	if alertsList[0].Component != "github.com/vyuvaraj/pranor/gate" || alertsList[0].Severity != "critical" {
		t.Errorf("alert field mismatch")
	}

	// 2. Update alert
	AddOrUpdateAlert("github.com/vyuvaraj/pranor/gate", "offline", "Pranor Gate is definitely offline", "critical")
	if len(alertsList) != 1 {
		t.Fatalf("expected 1 alert after update, got %d", len(alertsList))
	}
	if alertsList[0].Message != "Pranor Gate is definitely offline" {
		t.Errorf("expected updated message, got %q", alertsList[0].Message)
	}

	// 3. Clear alert
	ClearAlert("github.com/vyuvaraj/pranor/gate", "offline")
	if len(alertsList) != 0 {
...
func TestHandleAlertsEndpoint(t *testing.T) {
	alertsList := []Alert{
		{ID: "alert-1", Component: "github.com/vyuvaraj/pranor/vault", Type: "disk_full", Message: "Disk 95%", Severity: "warning"},
	}
	var mu sync.Mutex
...
func TestHandleAlertAckEndpoint(t *testing.T) {
	alertsList := []Alert{
		{ID: "alert-100", Component: "github.com/vyuvaraj/pranor/pulse", Type: "queue_oom", Message: "OOM", Severity: "critical", Acknowledged: false},
	}
	var mu sync.Mutex
	writeError := func(w http.ResponseWriter, r *http.Request, msg, code string, status int) {
		http.Error(w, msg, status)

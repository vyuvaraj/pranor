...
## Mission
Implement M4: ServPool features - Read/Write splitting (`rw_splitter.go`) and Connection Health Checker (`health_checker.go`), with unit tests and zero external dependency changes.
...
- **Success criteria**:
  - `go build ./...` and `go test ./...` in `packages/ServPool` pass with exit code 0.

...
func main() {
	portStr := flag.String("port", "8098", "Pranor Auth server port")
	flag.Parse()
	...
	mux.HandleFunc("/api/auth/federation/oidc", handlers.HandleTenantOidCFederation)
	...
	go func() {
		log.Printf("[INFO] Pranor Auth server starting on port %s", port)
		if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("failed to start Pranor Auth: %v", err)
		}
	}()
	...
	log.Println("[INFO] Pranor Auth server exited cleanly")
}

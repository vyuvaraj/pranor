...
func (p *LLMEdgeProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if !p.checkRateLimit() {
		http.Error(w, `{"error":{"message":"PranorGateway AI Rate Limit Exceeded (TPM/RPM threshold hit)","type":"rate_limit_error"}}`, http.StatusTooManyRequests)
		return
	}
	...
	if p.config.EnableCache && r.Method == http.MethodPost {
		...
		if found {
			...
			w.Header().Set("X-PranorGateway-AI-Cache", "HIT")
	...
	w.Header().Set("X-PranorGateway-AI-Cache", "MISS")
	w.Header().Set("X-PranorGateway-AI-Provider", string(p.config.Provider))

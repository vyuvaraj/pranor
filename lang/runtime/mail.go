...
func MailSend(toVal, templateVal, dataVal interface{}) interface{} {
	...
	LogInfo(fmt.Sprintf("[Pranor] [mail.send] To: %s, Template: %s, Context: %+v", to, templateStr, data))
	return true
}
...
func Notify(channelVal, targetVal, msgVal interface{}) interface{} {
	...
	LogInfo(fmt.Sprintf("[Pranor] [notify] Channel: %s, Target: %s, Message: %s", channel, target, msg))

	if strings.HasPrefix(notifyConnString, "Pranor Notify://") {
		baseURL := strings.Replace(notifyConnString, "Pranor Notify://", "http://", 1)
		...
		if err != nil {
			LogWarn("[Pranor] [notify] Failed to marshal payload: ", err.Error())
			return false
		}

		resp, err := http.Post(sendURL, "application/json", bytes.NewReader(payloadBytes))
		if err != nil {
			LogWarn("[Pranor] [notify] HTTP POST failed: ", err.Error())
			return false
		}
		...
		if resp.StatusCode >= 400 {
			...
			LogWarn("[Pranor] [notify] Pranor Notify returned error: ", string(bodyBytes))
			return false
		}
		return true
	}

...
import (
	...
	"github.com/vyuvaraj/pranor/trace/pkg/server"
	"github.com/vyuvaraj/pranor/trace/pkg/store"
)

func TestPranorTraceCollector(t *testing.T) {
		...
		"resourceSpans": []interface{}{
			map[string]interface{}{
				...
				"scopeSpans": []interface{}{
					map[string]interface{}{
						"scope": map[string]interface{}{"name": "Pranor-shared"},
						"spans": []interface{}{
							map[string]interface{}{
								"traceId":           traceID,
								"spanId":            span1ID,
...
func TestSamplingPolicies(t *testing.T) {
	...
	os.Setenv("PRANOR_TRACE_SAMPLING_RATE", "0")
	defer os.Unsetenv("PRANOR_TRACE_SAMPLING_RATE")
		...
		spans := []store.Span{
			{
				...
				Service:   "github.com/vyuvaraj/pranor/gate",
			},
		}
		s.AddSpans(spans)
	}
...
func TestAdaptiveSampling(t *testing.T) {
	os.Setenv("PRANOR_TRACE_SAMPLING_RATE", "10")
	defer os.Unsetenv("PRANOR_TRACE_SAMPLING_RATE")
...
func TestColdTierRetrievalLatency(t *testing.T) {
	...
	os.Setenv("PRANOR_CONFIG_S3_ENDPOINT", s3Mock.URL)
	defer os.Unsetenv("PRANOR_CONFIG_S3_ENDPOINT")

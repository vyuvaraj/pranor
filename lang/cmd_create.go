...
Your task is to write a single ".pnr" file that fulfills the user's requirements.
...
`
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		if mockResp := os.Getenv("PRANOR_TEST_AI_RESPONSE"); mockResp != "" {
		...
		code = strings.TrimPrefix(code, "```pranor")

...
func TestWebAuthnRegistrationAndAssertion(t *testing.T) {
	engine := NewWebAuthnEngine("servauth.dev", "ServAuth Identity Provider")
	...
	if opts.RP.ID != "servauth.dev" {
		t.Errorf("expected RP ID servauth.dev, got %s", opts.RP.ID)
	}
...
func TestWebAuthnExpiredOrInvalidChallenge(t *testing.T) {
	engine := NewWebAuthnEngine("servauth.dev", "ServAuth")

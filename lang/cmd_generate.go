...
func runGenerate() {
	if len(os.Args) < 3 {
		fmt.Println("Usage:")
		fmt.Println("  pranor generate client <file.pnr> [--lang <typescript|python|go>] [-o <output-file>]")
		fmt.Println("  pranor generate routes <spec.yaml|spec.json> [-o <output.pnr>]")
		fmt.Println("  pranor generate api <name>")
		fmt.Println("  pranor generate db <name>")
		fmt.Println("  pranor generate workflow <name>")
		fmt.Println("  pranor generate \"<natural-language-prompt>\"")
		os.Exit(1)
	}
	...
	case "api":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor generate api <name>")
			os.Exit(1)
		}
		runGenerateAPIScaffold(os.Args[3])
	case "db":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor generate db <name>")
			os.Exit(1)
		}
		runGenerateDBScaffold(os.Args[3])
	case "workflow":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor generate workflow <name> [--prompt <description>]")
			os.Exit(1)
		}
		prompt := ""
	...
	case "code":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor generate code <prompt>")
			os.Exit(1)
		}
		runGenerateCodePrompt(os.Args[3])
	case "sandbox":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor generate sandbox <file.pnr> [-o <output-file>]")
			os.Exit(1)
		}
...
func runGenerateClient() {
	...
	// Parse flags: pranor generate client [options] <file>
	var options []string
	var fileArg string
	...
	if fileArg == "" {
		fmt.Println("Usage: pranor generate client <file.pnr> [--lang <typescript|python|go>] [-o <output-file>]")
		os.Exit(1)
	}
...
`, name, strings.ToLower(name), name)
	outName := strings.ToLower(name) + "_api.pnr"
	if err := os.WriteFile(outName, []byte(code), 0644); err != nil {
...
`, name, name, strings.ToLower(name), name)
	outName := strings.ToLower(name) + "_db.pnr"
	if err := os.WriteFile(outName, []byte(code), 0644); err != nil {
...
`, name, name)
	outName := strings.ToLower(name) + "_workflow.pnr"
	if err := os.WriteFile(outName, []byte(code), 0644); err != nil {

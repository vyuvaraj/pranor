...
func initPlaygroundVars() {
	...
	candidates := []string{
		filepath.Join(pgRepoRoot, "pranor.exe"),
		filepath.Join(pgRepoRoot, "github.com/vyuvaraj/pranor/lang"),
	}
...
func runPlayground() {
	...
	fmt.Printf("🚀 pranor Playground Web IDE started at http://localhost:%d\n", port)
...
func handlePgRun(w http.ResponseWriter, r *http.Request) {
	...
	tempDir := filepath.Join(os.TempDir(), "PRANOR_playground_"+runId)
	...
	sourceFile := filepath.Join(tempDir, "main.pnr")

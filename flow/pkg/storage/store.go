...
func (s *PranorVaultWorkflowStore) LoadDefinitions() (map[string]WorkflowDef, error) {
	if s.Client == nil {
		return nil, errors.New("store client not initialized")
	}
	data, err := s.Client.Get("pranor-flow-state", "definitions.json")
	if err != nil {
		return nil, err
	}
	...
	log.Printf("[PERSISTENCE] Loaded %d workflow definitions from Pranor Vault", len(defs))
	return defs, nil
}

func (s *PranorVaultWorkflowStore) SaveDefinitions(defs map[string]WorkflowDef) error {
	if s.Client == nil {
	...
	return s.Client.Put("pranor-flow-state", "definitions.json", data)
}

func (s *PranorVaultWorkflowStore) LoadInstances() (map[string]*WorkflowInstance, error) {
	if s.Client == nil {
		return nil, errors.New("store client not initialized")
	}
	data, err := s.Client.Get("pranor-flow-state", "instances.json")
	if err != nil {
		return nil, err
	}
	...
	log.Printf("[PERSISTENCE] Loaded %d workflow instances from Pranor Vault", len(insts))
	return insts, nil
}

func (s *PranorVaultWorkflowStore) SaveInstances(insts map[string]*WorkflowInstance) error {
	if s.Client == nil {
		return errors.New("store client not initialized")
	}
	...
	return s.Client.Put("pranor-flow-state", "instances.json", data)
}

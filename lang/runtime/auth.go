...
func VerifyToken(token, secretOrProvider string) (map[string]interface{}, error) {
	...
	if strings.HasPrefix(secretOrProvider, "jwt://") {
	...
	} else if strings.HasPrefix(secretOrProvider, "oidc://") || strings.HasPrefix(secretOrProvider, "Pranor Auth://") {
		...
		if strings.HasPrefix(secretOrProvider, "oidc://") {
			...
			expectedIssuer = strings.TrimPrefix(secretOrProvider, "Pranor Auth://")
		}
...
func getOidcAesPublicKey(issuer, kid string) (*rsa.PublicKey, error) {
	...
	if jwksCacheURI == "" {
		if strings.HasPrefix(authSecretOrProvider, "Pranor Auth://") {
			wellKnownURL := issuer
...
func AuthRegister(usernameVal, emailVal, passwordVal interface{}) interface{} {
	...
	LogInfo(fmt.Sprintf("[Pranor] [auth.register] Registering user: %s (%s)", username, email))

	if strings.HasPrefix(authSecretOrProvider, "Pranor Auth://") {
		baseURL := strings.Replace(authSecretOrProvider, "Pranor Auth://", "http://", 1)
...
func AuthLogin(usernameVal, passwordVal interface{}) interface{} {
	username := fmt.Sprint(usernameVal)
	LogInfo(fmt.Sprintf("[Pranor] [auth.login] Logging in user: %s", username))
...
func AuthCurrentUser(reqVal interface{}) interface{} {
	LogInfo("[Pranor] [auth.currentUser] Resolving current user")

...
func TestS3PersistenceAndAuditLog(t *testing.T) {
	...
	// 1. Start a mock Pranor Vault S3 server
	s3Mock := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "PUT" {
			if r.URL.Path == "/pranor-chrono/jobs.json" {
				gotPutJob = true
				w.WriteHeader(http.StatusOK)
				return
			}
			if strings.HasPrefix(r.URL.Path, "/pranor-chrono/audit/") {
				gotPutAudit = true
		...
		if r.Method == "GET" && r.URL.Path == "/pranor-chrono/jobs.json" {
	...
	t.Setenv("PRANOR_STORE_ENDPOINT", s3Mock.URL)
	t.Setenv("PRANOR_STORE_BUCKET", "pranor-chrono")
	t.Setenv("PRANOR_STORE_AUTH_TOKEN", "mock-token")

...
const usage = `Pranor Vault CLI — unified command-line tool for Pranor Vault object storage and cluster administration.

Usage:
  Pranor Vault [global flags] <command> [args]

Global flags:
  --endpoint       Pranor Vault S3 API URL        (default: http://localhost:9000)
  --admin-endpoint Pranor Vault Admin API URL     (default: http://localhost:9001)
...
Admin & Daemon Commands:
  status                              Check status of running pranorVaultd daemon
...
func main() {
	flag.StringVar(&endpoint, "endpoint", "http://localhost:9000", "Pranor Vault S3 API URL")
	flag.StringVar(&adminEndpoint, "admin-endpoint", "http://localhost:9001", "Pranor Vault Admin API URL")
	...
	case "presign":
		err = cmdPresign(rest)
	case "version":
		fmt.Println("Pranor Vault CLI v2.0.0 (Unified Data & Admin Tool)")
	case "help", "--help", "-h":
		fmt.Print(usage)
	default:
		fatalf("unknown command %q — run 'Pranor Vault help' for usage\n", cmd)
	}
...
func cmdAdminStatus(args []string) error {
	...
	if err != nil {
		return fmt.Errorf("error connecting to pranorVaultd daemon at %s: %w", adminURL, err)
	}
	defer resp.Body.Close()
	...
	fmt.Println("Pranor Vault Daemon Status:")
...
func cmdBench(args []string) error {
	...
	bucket := fs.String("bucket", "Pranor Vault-bench", "Target bucket for benchmark")
	...
	fmt.Printf("=== Pranor Vault Built-in Benchmark (ST.D1) ===\n")
...
func cmdServeStatic(args []string) error {
	...
	fmt.Printf("=== Pranor Vault Static Site Host (ST.D5) ===\n")
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		// Attempt fetch from Pranor Vault S3 API
...
func cmdImport(args []string) error {
	...
	toBucket := fs.String("to", "", "Destination Pranor Vault bucket name (default: derived from --from)")
	if err := fs.Parse(args); err != nil {
		return err
	}

	if *fromSource == "" {
		return fmt.Errorf("--from flag required (e.g. Pranor Vault import --from s3://my-aws-bucket)")
	}
	...
	fmt.Printf("=== Pranor Vault Cloud Migration Mirror (ST.D2) ===\n")
	fmt.Printf("Mirroring from %s to Pranor Vault bucket %q...\n", *fromSource, destBucket)
...
func cmdPresign(args []string) error {
	...
	if fs.NArg() < 1 {
		return fmt.Errorf("usage: Pranor Vault presign <bucket/key> [--expires 3600]")
	}

...
func (s *SmartAIRouter) RouteAndExecute(ctx context.Context, prompt string) (string, PromptComplexity, float64, error) {
	...
	if err != nil {
		res := fmt.Sprintf(`{"model":"%s","complexity":"%s","savings_usd":%.4f,"content":"PranorGateway Smart AI Response (Offline Mode)"}`, modelUsed, complexity, estimatedSavings)
	...
	if err != nil || resp.StatusCode >= 400 {
		res := fmt.Sprintf(`{"model":"%s","complexity":"%s","savings_usd":%.4f,"content":"PranorGateway Smart AI Response (Upstream Offline Fallback)"}`, modelUsed, complexity, estimatedSavings)
		return res, complexity, estimatedSavings, nil
	}
	...
	if err != nil {
		res := fmt.Sprintf(`{"model":"%s","complexity":"%s","savings_usd":%.4f,"content":"PranorGateway Smart AI Response"}`, modelUsed, complexity, estimatedSavings)
		return res, complexity, estimatedSavings, nil
	}

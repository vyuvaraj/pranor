...
import (
	...
	"github.com/vyuvaraj/pranor/notify/pkg/delivery"
...
func main() {
	if len(os.Args) > 1 && os.Args[1] == "test-send" {
		...
		subject := fs.String("subject", "Pranor Notify Test Email", "Email subject line")
		body := fs.String("body", "This is a test email sent via Pranor Notify CLI.", "Email body content")
		_ = fs.Parse(os.Args[2:])

		log.Printf("=== Pranor Notify Test-Send CLI ===")
		log.Printf("To: %s", *to)
	...
	portStr := flag.String("port", "8094", "Pranor Notify server port")
	...
	go func() {
		log.Printf("[INFO] Pranor Notify server starting on port %s", port)
		if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("failed to start Pranor Notify: %v", err)
		}
	}()
	...
	log.Println("[INFO] Pranor Notify server exited cleanly")
}

...
func NewServGatewayDaemon(configPath string) (*ServGatewayDaemon, error) {
	cfg := GatewayConfig{
		Addr:           ":8080",
		AdminAddr:      ":8081",
		EnableWebAdmin: true,
...
func (d *ServGatewayDaemon) Start() error {
	d.mu.Lock()
	...
	log.Printf("[servgatewayd] Standalone ServGateway Daemon listening on %s", d.config.Addr)
	return d.server.ListenAndServe()
}

func (d *ServGatewayDaemon) createHTTPHandler() http.Handler {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if !exists {
			http.Error(w, fmt.Sprintf("ServGateway: no route for path %s", r.URL.Path), http.StatusNotFound)
			return
		}

		w.Header().Set("X-ServGateway-Version", "v2.0.0")
		w.Header().Set("X-ServGateway-Upstream", route.Target)
		w.WriteHeader(http.StatusOK)
...
func (d *ServGatewayDaemon) handleHealth(w http.ResponseWriter, r *http.Request) {
...
func (d *ServGatewayDaemon) handleRoutes(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	d.mu.RLock()
	defer d.mu.RUnlock()
...
func (d *ServGatewayDaemon) handleWebAdminUI(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	html := `<!DOCTYPE html>
<html>
<head>
    <title>ServGateway Inspector UI</title>
    <style>
...
<body>
    <div class="card">
        <h1>⚡ ServGateway Admin Console</h1>
...
func (d *ServGatewayDaemon) Shutdown(ctx context.Context) error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func RunDaemonSignalHandler(d *ServGatewayDaemon) {

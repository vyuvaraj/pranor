...
func (s *Server) registerWithGateway(name string, port int) {
	// Map to Pranor Gate Route format:
	// Prefix = "/service/" + name
...
func (s *Server) deregisterFromGateway(name string) {
	...
	// Pranor Gate's API does not expose a DELETE for routes directly, but we can query then post back the list without the prefix, 
...
func (s *Server) handleCgroupResourceLimits(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodPost || r.Method == http.MethodPut {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"cgroup_path":        fmt.Sprintf("/sys/fs/cgroup/Pranor Deploy/%s", req.Service),

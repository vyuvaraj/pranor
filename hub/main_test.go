...
func TestMarketplace(t *testing.T) {
	...
	// 2. Publish new item
	newItem := `{"name":"custom-workflow","version":"2.1.0","type":"workflow","description":"Test custom workflow template","publisher":"Alice","url":"https://pranor.dev/marketplace/custom-workflow.json"}`
...
func TestSignatureTamperDetection(t *testing.T) {
	...
	buildTarball := func(content string) []byte {
		...
		tw := tar.NewWriter(gw)
		toml := content
		hdr := &tar.Header{Name: "pranor.toml", Size: int64(len(toml)), Mode: 0644}
		tw.WriteHeader(hdr)
		tw.Write([]byte(toml))
		tw.Close()
		gw.Close()

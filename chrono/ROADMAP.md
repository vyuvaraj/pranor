# Pranor Chrono Roadmap

This roadmap outlines the planned development phases for the Pranor Chrono distributed scheduling service.

---

## Differentiating Factors (Why Pranor Chrono?)

* **Language-Native Synchronization**: Designed to coordinate Pranor's declarative `every` and `cron` syntax code blocks across multiple replicated microservice pods.
...
* **OTel Trace Correlation**: Standard header propagation (`traceparent`) automatically hooks job triggers into OTel context, allowing operations to trace execution duration and failures in `Pranor Console`.
...
## Phase 3: Pranor Vault Backing (Q4 2026)
- [x] **Persistent Job Registry**: Back job state persistence to a `Pranor Vault` S3 bucket, preventing scheduled task losses during server crashes or restarts.
...
- [ ] **Job Chaining via Pranor Pulse**: Trigger next job by publishing to Pranor Pulse topic on completion. Event-driven scheduling.
...
- [ ] **Cron Expression Builder UI**: Visual cron builder in Pranor Console with next-5-runs preview.

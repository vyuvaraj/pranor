...
func runDoctor(integration bool) {
	...
	if err := json.Unmarshal([]byte(raw), &discovery); err != nil {
		...
		if err := json.Unmarshal(data, &discovery); err != nil {
			fmt.Printf("❌ Error: failed to parse PRANOR_DISCOVERY file: %v\n", err)
			osExit(1)
		}
	}
	...
	services := []struct {
		...
		{"github.com/vyuvaraj/pranor/cache", discovery.Cache},
		{"github.com/vyuvaraj/pranor/chrono", discovery.Cron},
		{"github.com/vyuvaraj/pranor/mesh", discovery.Mesh},
		{"github.com/vyuvaraj/pranor/deploy", discovery.Cloud},
		{"github.com/vyuvaraj/pranor/tunnel", discovery.Tunnel},
		{"github.com/vyuvaraj/pranor/trace", discovery.Trace},
		{"github.com/vyuvaraj/pranor/hub", discovery.Registry},
...
func checkTelemetryPipeline() {
	...
	otlp := os.Getenv("PRANOR_OTLP_ENDPOINT")
	if otlp == "" {
		fmt.Println("⚠️ Warning: PRANOR_OTLP_ENDPOINT environment variable is not set. Services will not emit traces.")
		return
	}

...
var (
	OidcIssuer       = os.Getenv("PRANOR_OIDC_ISSUER")
	OidcClientID     = os.Getenv("PRANOR_OIDC_CLIENT_ID")
	OidcClientSecret = os.Getenv("PRANOR_OIDC_CLIENT_SECRET")
	OidcRedirectURL  = os.Getenv("PRANOR_OIDC_REDIRECT_URL")
...
func Init(auditLogFunc func(string, string, string, string, int)) {
	...
	if sec == "" {
		sec = fmt.Sprintf("ephemeral-%d-%d", time.Now().UnixNano(), sha256.Sum256([]byte("Pranor Console-salt")))
		log.Println("[auth] PRANOR_JWT_SECRET not set. Generated ephemeral session key.")
	}
...
func AuthorizeConsole(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		jwtSec := os.Getenv("PRANOR_JWT_SECRET")

...
func TestPranorGateReverseProxy(t *testing.T) {
...
func TestInstallCommand(t *testing.T) {
	...
	os.Args = []string{"pranor-gate", "install", "auth-token"}
...
func TestPranorGateWebhookBridge(t *testing.T) {
	// 1. Setup mock Pranor Pulse server
	receivedPayload := ""
	receivedTopic := ""
...
`
	...
	oldArgs := os.Args
	defer func() { os.Args = oldArgs }()
	os.Args = []string{"pranor-gate", "policy", "compile", tmpPolicyFile.Name(), "-o", tmpWasmFile.Name()}
	runPolicyCommand()
...
func TestConfigSignatureVerification(t *testing.T) {
	os.Setenv("PRANOR_JWT_SECRET", "super-secret-key-12345")
...
func TestGitOpsConfigSyncWebhook(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "Pranor Gate-gitops-*.json")
	if err != nil {
...
func TestDynamicIAMPolicyHotReloading(t *testing.T) {
	...
	t.Setenv("PRANOR_JWT_SECRET", jwtSec)
...
func TestTenantControlPlanePolicies(t *testing.T) {
	os.Setenv("PRANOR_CLUSTER", "prod-cluster-1")
	os.Setenv("PRANOR_REGION", "eu-west")
	defer os.Unsetenv("PRANOR_CLUSTER")
	defer os.Unsetenv("PRANOR_REGION")
...
func TestEcosystemSchemaRegistryValidation(t *testing.T) {
	...
	os.Setenv("PRANOR_REGISTRY_URL", registryMock.URL)
	defer os.Unsetenv("PRANOR_REGISTRY_URL")

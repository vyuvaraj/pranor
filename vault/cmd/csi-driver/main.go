...
func (d *CSIDriver) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	_ = os.Remove(fmt.Sprintf("%s/Pranor Vault-csi.config", targetPath))
...
func main() {
	...
	if endpoint == "" {
		endpoint = "unix:///var/lib/kubelet/plugins/csi.Pranor Vault.io/csi.sock"
	}

	fmt.Printf("Starting Pranor Vault CSI Driver on endpoint: %s\n", endpoint)
	...
	<-stop
	server.GracefulStop()
	fmt.Println("Pranor Vault CSI Driver stopped gracefully")
}

...
func runIntegrationTests(pnrFile string, withCoverage bool, filter string) bool {
	...
	services := []integrationService{
		{
			...
			Binary: "github.com/vyuvaraj/pranor/vault",
			Port:   storePort,
			Args:   []string{"--port", fmt.Sprintf("%d", storePort), "--data-dir", tmpDir + "/store"},
		},
		{
			Name:   "github.com/vyuvaraj/pranor/pulse",
			Binary: "github.com/vyuvaraj/pranor/pulse",
			Port:   queuePort,
		},
		{
			Name:   "github.com/vyuvaraj/pranor/cache",
			Binary: "github.com/vyuvaraj/pranor/cache",
			Port:   cachePort,
		},
	}
	...
	os.Setenv("PRANOR_STORE_ENDPOINT", fmt.Sprintf("http://localhost:%d", storePort))
	os.Setenv("PRANOR_QUEUE_ENDPOINT", fmt.Sprintf("localhost:%d", queuePort))
	os.Setenv("PRANOR_CACHE_ENDPOINT", fmt.Sprintf("http://localhost:%d", cachePort))
	os.Setenv("PRANOR_INTEGRATION_TEST", "true")

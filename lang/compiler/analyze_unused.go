...
func analyzeCrossServiceDeadCode(program *Program) []Diagnostic {
	...
	if err != nil {
		// Pranor Mesh registry is not reachable or offline, bypass warning diagnostics
		return nil
	}
	conn.Close()

	// If Pranor Mesh is online, we warn if local routes aren't registered/called in the mesh

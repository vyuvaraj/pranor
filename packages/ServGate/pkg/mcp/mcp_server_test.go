...
func TestMCPInitialize(t *testing.T) {
	s := NewMCPServer("test-servverse", "1.0.0")
	...
	if si["name"] != "test-servverse" {
		t.Errorf("expected server name 'test-servverse', got %v", si["name"])
	}
...
func TestMCPToolsCall(t *testing.T) {
	...
	resp := mcpRPC(t, s.Handler(), "tools/call", map[string]interface{}{
		"name":      "greet",
		"arguments": map[string]interface{}{"name": "servverse"},
	})
	result, ok := resp["result"].(map[string]interface{})
	if !ok {
	...
	if !strings.Contains(item["text"].(string), "servverse") {
		t.Errorf("expected greeting to contain 'servverse', got: %v", item["text"])
	}
}

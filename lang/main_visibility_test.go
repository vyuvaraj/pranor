...
func TestVisibilityEnforcement(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "PRANOR_visibility_test")
	if err != nil {
...
`
	helperPath := filepath.Join(tmpDir, "helper.pnr")
	if err := os.WriteFile(helperPath, []byte(helperContent), 0644); err != nil {
		t.Fatalf("failed to write helper.pnr: %v", err)
	}

	// Case 1: Valid selective import (User and createUser)
	main1Content := `
import { User, createUser } from "./helper.pnr"
`
	main1Path := filepath.Join(tmpDir, "main1.pnr")
	if err := os.WriteFile(main1Path, []byte(main1Content), 0644); err != nil {
		t.Fatalf("failed to write main1.pnr: %v", err)
	}
	...
	main2Content := `
import { User, privateHelper } from "./helper.pnr"
`
	main2Path := filepath.Join(tmpDir, "main2.pnr")
	if err := os.WriteFile(main2Path, []byte(main2Content), 0644); err != nil {
		t.Fatalf("failed to write main2.pnr: %v", err)
	}
	...
	main3Content := `
import { User, fooBar } from "./helper.pnr"
`
	main3Path := filepath.Join(tmpDir, "main3.pnr")
	if err := os.WriteFile(main3Path, []byte(main3Content), 0644); err != nil {
		t.Fatalf("failed to write main3.pnr: %v", err)
	}
...
import "./helper.pnr"
`
	main4Path := filepath.Join(tmpDir, "main4.pnr")
	if err := os.WriteFile(main4Path, []byte(main4Content), 0644); err != nil {
		t.Fatalf("failed to write main4.pnr: %v", err)
	}

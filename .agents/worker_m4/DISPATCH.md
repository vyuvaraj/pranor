...
4. Run `go build ./...` and `go test ./...` in `/home/developer/workspace/serv/packages/Pranor Pool`. Ensure exit code 0.
5. Verify `git diff go.mod` in `packages/Pranor Pool` shows NO external dependency changes.
6. Write `changes.md` and `handoff.md` in `/home/developer/workspace/serv/.agents/worker_m4`.

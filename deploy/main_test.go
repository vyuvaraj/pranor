...
func TestPranorDeployLifecycle(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "Pranor Deploy-test-*")
	if err != nil {
...
func TestPranorDeployPhase3Features(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "Pranor Deploy-phase3-test-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
...
func TestOrchestratorIsolationModes(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "Pranor Deploy-test-modes-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
...
func TestRollingDeployments(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "Pranor Deploy-test-rolling-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
...
func TestEnvVariablesManagement(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "Pranor Deploy-test-env-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
...
func TestHorizontalAutoScaling(t *testing.T) {
	if !server.IsAutoscaleSupported {
		t.Skip("Skipping: auto-scaling requires Pranor Deploy Enterprise Edition")
	}

	tempDir, err := os.MkdirTemp("", "Pranor Deploy-autoscale-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)
...
OuterLoop:
	...
	if len(targets) < 2 {
		t.Errorf("Expected scale-app to scale up and have at least 2 targets in Pranor Gate route, got targets: %v", targets)
	} else {
...
func TestScaleToZeroAndInvoke(t *testing.T) {
	if !server.IsAutoscaleSupported {
		t.Skip("Skipping: scale-to-zero requires Pranor Deploy Enterprise Edition")
	}

	tempDir, err := os.MkdirTemp("", "Pranor Deploy-scale-to-zero-*")
	if err != nil {
		t.Fatalf("failed to create temp dir: %v", err)
	}
	defer os.RemoveAll(tempDir)

...
func TestServLangNext5RoadmapItems(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_next5_*.pnr")
	if err != nil {
...
func TestPranorPulseStreamDSL(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_stream_dsl_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
...
func TestEcosystemNext4RoadmapItems(t *testing.T) {
	tmpFile, err := os.CreateTemp("", "test_next4_*.pnr")
	if err != nil {
		t.Fatalf("failed to create temp file: %v", err)
	}
	defer os.Remove(tmpFile.Name())
...
func parseProjectString(content string) (string, *compiler.Program, error) {
	tmpFile, err := os.CreateTemp("", "test_parse_*.pnr")
	if err != nil {
		return "", nil, err
	}
	defer os.Remove(tmpFile.Name())

...
func runTunnelCmd() {
	...
	if len(os.Args) < 3 {
		fmt.Println("Usage: pranor tunnel <local-port> [options]")
		fmt.Println("\nOptions:")
...
func runTunnelInspect() {
	...
	if err != nil {
		fmt.Printf("Failed to connect to Pranor Tunnel relay at %s: %v\n", relayHost, err)
		os.Exit(1)
	}
	...
	if tResp.StatusCode != http.StatusOK {
		...
		fmt.Printf("Pranor Tunnel relay returned error status %d: %s\n", tResp.StatusCode, string(body))
		os.Exit(1)
	}
	...
	fmt.Println("=== Pranor Tunnel Active Connections ===")

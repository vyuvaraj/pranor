package import (
	...
	"github.com/vyuvaraj/pranor/console/pkg/tabs"
	"github.com/vyuvaraj/pranor/console/pkg/topology"
	pkgalerts "github.com/vyuvaraj/pranor/console/pkg/alerts"
	pkgdashboards "github.com/vyuvaraj/pranor/console/pkg/dashboards"
	"github.com/vyuvaraj/pranor/console/pkg/launcher"
	"github.com/vyuvaraj/pranor/console/pkg/incidents"
	"github.com/vyuvaraj/pranor/console/pkg/provision"
	"github.com/vyuvaraj/pranor/console/pkg/ai"
)
...
func TestHandleTraceReplay(t *testing.T) {
	...
	*gateUrl = mockGateSrv.URL
	...
	if receivedMethod != "POST" || receivedBody != `{"hello":"world"}` {
		t.Errorf("request was not replayed correctly to Pranor Gate: method=%s body=%s", receivedMethod, receivedBody)
	}
}
...
func TestHandleRunbookExecution(t *testing.T) {
	req := httptest.NewRequest("GET", "/api/runbooks?component=github.com/vyuvaraj/pranor/gate", nil)
	w := httptest.NewRecorder()
	...
	if len(actions) < 1 {
		t.Fatal("expected at least one runbook action for github.com/vyuvaraj/pranor/gate")
	}
...
func TestHandleDiagnosticExec(t *testing.T) {
	payload := `{"service":"github.com/vyuvaraj/pranor/gate","command":"ps aux"}`
	...
	if !strings.Contains(res["output"].(string), "pranor github.com/vyuvaraj/pranor/gate") {
		t.Errorf("expected output to contain command result, got: %s", res["output"])
	}
}

func TestHandleTopologyLive(t *testing.T) {
	// Start a mock Pranor Vault that returns trace spans
			...
			spans := []map[string]any{
				{
					...
					"SpanID":       "span-001",
					"ParentSpanID": "",
					"ServiceName":  "github.com/vyuvaraj/pranor/gate",
					"DurationNs":   15000000,
					"StatusCode":   "ok",
				...
				},
				{
					...
					"ServiceName":  "github.com/vyuvaraj/pranor/vault",
					"DurationNs":   8000000,
					"StatusCode":   "ok",
				...
				},
				{
					...
					"SpanID":       "span-003",
					"ParentSpanID": "span-001",
					"ServiceName":  "github.com/vyuvaraj/pranor/pulse",
					"DurationNs":   3000000,
					"StatusCode":   "error",
	...
	*storeUrl = mockStore.URL
	...
	// Verify Pranor Gate and Pranor Vault appear in nodes
	foundGate, foundStore := false, false
	for _, n := range result.Nodes {
		if n.ID == "github.com/vyuvaraj/pranor/gate" {
			foundGate = true
			if n.HealthScore < 0 || n.HealthScore > 1 {
				t.Errorf("health score out of range: %f", n.HealthScore)
			}
		}
		if n.ID == "github.com/vyuvaraj/pranor/vault" {
			foundStore = true
		}
	}
	if !foundGate {
		t.Error("expected Pranor Gate node in topology")
	}
	if !foundStore {
		t.Error("expected Pranor Vault node in topology")
	}
...
func TestHandleDashboards(t *testing.T) {
	...
	payload := `{"name":"SRE Alerts Board","description":"Custom board for SRE team","widgets":[{"id":"w1","title":"Error Spike","metric":"error_rate","chart_type":"line","time_range":"1h","service":"github.com/vyuvaraj/pranor/gate","width":6,"height":4}],"shared_with":["sre-team"]}`
...
func TestDesignerAndStudioEndpoints(t *testing.T) {
	...
	debugPayload := `{"action":"set_breakpoint","file":"main.pnr","line":12}`
...
func TestConsoleV1Readiness(t *testing.T) {
	...
	if ver["service"] != "github.com/vyuvaraj/pranor/console" || ver["version"] != "1.0.0" {

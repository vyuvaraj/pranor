...
func runTestsWithWriter(pnrFile string, withCoverage bool, filter string, stdoutWriter io.Writer, stderrWriter io.Writer) bool {
	...
	mainStub := "// Code generated by Pranor compiler. DO NOT EDIT.\npackage main\n\nfunc main() {}\n"
	...
	// Write PRANOR_test.go
	testCode := cg.GenerateTests()
	testFile := filepath.Join(buildDir, "PRANOR_test.go")
	...
	if os.Getenv("PRANOR_ENV") == "" {
		cmd.Env = append(cmd.Env, "PRANOR_ENV=test")
	}
	...
	if testErr != nil {
		...
		if tidyErr := runGoModTidy(buildDir); tidyErr == nil {
			...
			if os.Getenv("PRANOR_ENV") == "" {
				retryCmd.Env = append(retryCmd.Env, "PRANOR_ENV=test")
			}
			if os.Getenv("TESTING") == "" {
	...
	if testErr != nil {
		...
		if data, err := os.ReadFile(testFile); err == nil {
			fmt.Fprintln(stderrWriter, "--- Generated PRANOR_test.go ---")

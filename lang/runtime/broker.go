...
func InitBroker(url string) {
	...
	if strings.HasPrefix(url, "nats://") {
		...
		if err != nil {
			LogWarn("Failed to connect to STOMP/Pranor Pulse broker: ", err, " - Falling back to in-memory broker")
		} else {
			LogInfo("Connected to STOMP/Pranor Pulse broker successfully at ", addr)
		}
...
func RegisterPranorPulseDLQ(topic string, dlqTopic string) {
	...
	if err != nil {
		LogWarn("Failed to register DLQ on Pranor Pulse via HTTP:", err)
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		LogWarn("Pranor Pulse DLQ registration returned status ", resp.StatusCode, ": ", string(bodyBytes))
	} else {
		LogInfo("Successfully registered DLQ ", dlqTopic, " for topic ", topic, " on Pranor Pulse")
	}
}
...
func EmitEvent(streamName string, eventType string, payload interface{}) {
	...
	queueAddr := os.Getenv("PRANOR_QUEUE_URL")
	if queueAddr == "" {
...
func SnapshotAggregate(streamName string, id string, state interface{}) {
	...
	storeAddr := os.Getenv("PRANOR_STORE_URL")
	if storeAddr == "" {

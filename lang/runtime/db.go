...
func uploadSchemaToPranorVault() {
	time.Sleep(1 * time.Second)

	storeURL := os.Getenv("PRANOR_STORE_URL")
	if storeURL == "" {
		if discovery := os.Getenv("PRANOR_DISCOVERY"); discovery != "" {
	...
	serviceName := os.Getenv("PRANOR_SERVICE_NAME")
	if serviceName == "" {
	...
	if err != nil {
		LogWarn("Failed to upload DB schema to Pranor Vault: ", err.Error())
		return
	}
	defer resp.Body.Close()
	if resp.StatusCode == http.StatusOK {
		LogInfo("Successfully registered database ORM schema to Pranor Vault")
	} else {
		LogWarn("Pranor Vault schema registration returned status ", resp.StatusCode)
	}
}

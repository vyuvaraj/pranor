...
// runMigrate parses all .pnr files in the target path, extracts TableDecl nodes,
// compares them against the live database schema, and applies any missing tables
// or columns as a structural migration.
//
// Usage: pranor migrate [file-or-dir] [--db <connection-string>]
// Usage: pranor migrate [file-or-dir] [--db <connection-string>] [--rollback]
// Usage: pranor migrate [file-or-dir] [--db <connection-string>]
// Usage: pranor migrate [file-or-dir] [--db <connection-string>] [--rollback]
// Usage: pranor migrate [file-or-dir] [--db <connection-string>] [--dry-run]
func runMigrate(target string, dbConn string, rollback bool, dryRun bool) {
	...
	if !dryRun {
		if _, err := db.Exec(`CREATE TABLE IF NOT EXISTS PRANOR_schema_migrations (
...
// collectTableDecls parses the given .pnr files and returns all TableDecl nodes.
...
func findPnrFiles(target string) ([]string, error) {
	...
	for _, entry := range entries {
		if !entry.IsDir() && filepath.Ext(entry.Name()) == ".pnr" {
...
func runMigrateStatus(target, dbConn string) {
	...
	fmt.Printf("=== Pranor Migration Status (SL.H4) ===\n")
	fmt.Printf("Database: %s\n\n", dbConn)

	rows, err := db.Query("SELECT table_name, applied_at FROM PRANOR_schema_migrations ORDER BY applied_at ASC")
	if err != nil {
		fmt.Println("No migration tracking table found (`PRANOR_schema_migrations`). Run `pranor migrate` first.")
		return
	}
	defer rows.Close()

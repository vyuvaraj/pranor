...
import (
	...
	"github.com/vyuvaraj/pranor/mesh/pkg/client"
	"github.com/vyuvaraj/pranor/mesh/pkg/registry"
	"github.com/vyuvaraj/pranor/mesh/pkg/resilience"
...
func TestPranorMeshLifecycle(t *testing.T) {
	...
	// 3. Make requests to pranor://user-service and check load balancing
	for i := 0; i < 4; i++ {
		resp, err := httpClient.Get("pranor://user-service/users")
		if err != nil {
	...
	for i := 0; i < 3; i++ {
		_, _ = failClient.Get("pranor://failing-service/")
	}

	// Next request should fail immediately with ErrCircuitOpen/circuit open error
	_, err := failClient.Get("pranor://failing-service/")
...
func TestRegistryJWTAuthentication(t *testing.T) {
	jwtSecret := "my-secret"
	os.Setenv("PRANOR_JWT_SECRET", jwtSecret)
	defer os.Unsetenv("PRANOR_JWT_SECRET")
...
func TestDynamicMTLS(t *testing.T) {
	...
	resp, err := httpClient.Get("pranor://my-secure-service/secure")
	if err != nil {
...
func TestCanaryTrafficSplitting(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		resp, err := httpClient.Get("pranor://canary-service/endpoint")
		if err != nil {
			t.Fatalf("request failed: %v", err)
		}
		resp.Body.Close()
...
func TestChaosFaultInjection(t *testing.T) {
	...
	start := time.Now()
	res, err := httpClient.Get("pranor://chaos-service/some-endpoint")
...
func TestGRPCMeshTransport(t *testing.T) {
	os.Setenv("PRANOR_MESH_GRPC", "true")
	defer os.Unsetenv("PRANOR_MESH_GRPC")
	...
	req, _ := http.NewRequest("GET", "pranor://grpc-service/hello", nil)
...
func TestZeroTrustNetworkPolicies(t *testing.T) {
	...
	req1, _ := http.NewRequest("GET", "pranor://secure-db/api/db/read", nil)
	...
	req2, _ := http.NewRequest("POST", "pranor://secure-db/api/db/write", nil)
	...
	req3, _ := http.NewRequest("GET", "pranor://secure-db/api/db/read", nil)
...
func TestPranorMeshRateLimitingPerPair(t *testing.T) {
	...
	rateLimitedCount := 0
	for i := 0; i < 5; i++ {
		req, _ := http.NewRequest("GET", "pranor://rate-svc/ping", nil)
...
func TestPranorMeshVersionRouting(t *testing.T) {
	// Start a control plane registry
	reg := registry.NewRegistry(5 * time.Second)
	defer reg.Close()
	regServer := httptest.NewServer(reg.Handler())
	...
	for i := 0; i < 4; i++ {
		req, _ := http.NewRequest("GET", "pranor://versioned-svc/data", nil)
	...
	v2Count = 0
	for i := 0; i < 4; i++ {
		r, err := httpClient.Get("pranor://versioned-svc/data")
		if err != nil {
...
func TestHealthMetricsPush(t *testing.T) {
	...
	httpClient := &http.Client{Transport: transport}
	for i := 0; i < 4; i++ {
		resp, err := httpClient.Get("pranor://health-svc/ping")
		if err != nil {
			t.Fatalf("request %d failed: %v", i, err)
		}
		resp.Body.Close()
...
func TestGRPCResolverAndInterceptor(t *testing.T) {
	...
	if builder.Scheme() != "pranor" {
		t.Errorf("expected scheme 'pranor', got '%s'", builder.Scheme())
	}

...
// EmitAuditEvent writes an audit event JSON payload to the system-audit-logs bucket in Pranor Vault.
func EmitAuditEvent(service, action, actor string, details map[string]interface{}) error {
	...
	retentionDaysStr := os.Getenv("PRANOR_AUDIT_RETENTION_DAYS")
	if retentionDaysStr != "" {

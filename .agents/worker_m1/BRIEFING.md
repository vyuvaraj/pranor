...
## Mission
Implement M1 (ServAuth): TokenStore (SA.G1) and VelocityLimiter (SA.G6) with comprehensive unit tests and zero external dependency changes.
...
- Run `go build ./...` and `go test ./...` in `packages/ServAuth` with exit code 0
...
## Task Summary
- **What to build**: `TokenStore` in `packages/ServAuth/pkg/sessions/token_store.go` and `VelocityLimiter` in `packages/ServAuth/pkg/security/velocity_limiter.go`

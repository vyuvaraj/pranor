...
func TestSelfHealingObservabilityLoop(t *testing.T) {
	...
	// 1. Start Mock Pranor Deploy server
	...
	os.Setenv("PRANOR_CLOUD_URL", cloudMock.URL)
	defer os.Unsetenv("PRANOR_CLOUD_URL")

	// 2. Initialize Pranor Trace Store & Server
	...
	if !rollbackTriggered {
		t.Fatal("Self-healing did not trigger rollback for payment-service on Pranor Deploy")
	}

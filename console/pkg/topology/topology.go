...
func HandleTopology(w http.ResponseWriter, r *http.Request) {
	...
	nodesMap["github.com/vyuvaraj/pranor/gate"] = &TopologyNode{ID: "github.com/vyuvaraj/pranor/gate", Label: "Pranor Gate (Gateway)", Color: "#06b6d4", Online: true}
	nodesMap["github.com/vyuvaraj/pranor/vault"] = &TopologyNode{ID: "github.com/vyuvaraj/pranor/vault", Label: "Pranor Vault (Storage)", Color: "#10b981", Online: true}
	nodesMap["github.com/vyuvaraj/pranor/pulse"] = &TopologyNode{ID: "github.com/vyuvaraj/pranor/pulse", Label: "Pranor Pulse (Broker)", Color: "#f59e0b", Online: true}
	nodesMap["github.com/vyuvaraj/pranor/tunnel"] = &TopologyNode{ID: "github.com/vyuvaraj/pranor/tunnel", Label: "Pranor Tunnel (Relay)", Color: "#6366f1", Online: true}
	...
	for _, span := range spans {
		...
		if strings.Contains(span.Name, "PUT") || strings.Contains(span.Name, "GET") {
			edgeKey := fmt.Sprintf("%s->Pranor Vault", svc)
			if _, exists := edgesMap[edgeKey]; !exists {
				edgesMap[edgeKey] = &TopologyEdge{
					From:      svc,
					To:        "github.com/vyuvaraj/pranor/vault",
					Label:     "S3",
					LatencyMs: latMs,
				}
			}
		...
		if strings.Contains(span.Name, "publish") || strings.Contains(span.Name, "subscribe") {
			edgeKey := fmt.Sprintf("%s->Pranor Pulse", svc)
			if _, exists := edgesMap[edgeKey]; !exists {
				edgesMap[edgeKey] = &TopologyEdge{
					From:      svc,
					To:        "github.com/vyuvaraj/pranor/pulse",
					Label:     "STOMP",
					LatencyMs: latMs,
				}
			}
	...
	for _, n := range nodes {
		if n.ID != "github.com/vyuvaraj/pranor/gate" && n.ID != "github.com/vyuvaraj/pranor/vault" && n.ID != "github.com/vyuvaraj/pranor/pulse" && n.ID != "github.com/vyuvaraj/pranor/tunnel" {
			edgeKey := fmt.Sprintf("Pranor Gate->%s", n.ID)
			if _, exists := edgesMap[edgeKey]; !exists {
				edges = append(edges, TopologyEdge{
					From:      "github.com/vyuvaraj/pranor/gate",
					To:        n.ID,
					Label:     "HTTP",
					LatencyMs: 10,
				})
...
func HandleTopologyLive(w http.ResponseWriter, r *http.Request) {
	...
	serviceColors := map[string]string{
		"github.com/vyuvaraj/pranor/gate":   "#06b6d4",
		"github.com/vyuvaraj/pranor/vault":  "#10b981",
		"github.com/vyuvaraj/pranor/pulse":  "#f59e0b",
		"github.com/vyuvaraj/pranor/tunnel": "#6366f1",
		"github.com/vyuvaraj/pranor/trace":  "#a855f7",
	}
	...
	for _, span := range spans {
		...
		if strings.Contains(span.Name, "PUT") || strings.Contains(span.Name, "GET") || strings.Contains(span.Name, "DELETE") {
			if svc != "github.com/vyuvaraj/pranor/vault" {
				edgeKey := svc + "->Pranor Vault"
				if _, exists := edgeStatsMap[edgeKey]; !exists {
		...
		if strings.Contains(span.Name, "publish") || strings.Contains(span.Name, "subscribe") {
			if svc != "github.com/vyuvaraj/pranor/pulse" {
				edgeKey := svc + "->Pranor Pulse"
				if _, exists := edgeStatsMap[edgeKey]; !exists {
					edgeStatsMap[edgeKey] = &edgeStats{totalLatency: latMs, requestCount: 1}
				}
			}
	...
	for edgeKey, es := range edgeStatsMap {
		...
		label := "Call"
		if parts[1] == "github.com/vyuvaraj/pranor/vault" {
			label = "S3"
		} else if parts[1] == "github.com/vyuvaraj/pranor/pulse" {
			label = "STOMP"
		} else if parts[0] == "github.com/vyuvaraj/pranor/gate" {
			label = "HTTP"
		}
...
func HandleTraceReplay(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteJSONError(w, r, fmt.Sprintf("Failed to fetch trace from Pranor Trace: %v", err), "ERR_FETCH_TRACE_FAILED", http.StatusInternalServerError)
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		WriteJSONError(w, r, "Trace not found in Pranor Trace", "ERR_TRACE_NOT_FOUND", http.StatusNotFound)
		return
	}
	...
	if err != nil {
		json.NewEncoder(w).Encode(ReplayResponse{
			Success: false,
			Error:   fmt.Sprintf("Failed to replay request through Pranor Gate: %v", err),
		})
		return
	}
...
func HandleTraceWaterfall(w http.ResponseWriter, r *http.Request) {
	...
	root := WaterfallSpan{
		...
		Children: []WaterfallSpan{
			{
				ID:       "span-auth",
				Name:     "Pranor Auth /api/auth/keys/validate",
				Duration: 15,
				Offset:   5,
			},
			{
			...
			{
				ID:       "span-queue-pub",
				Name:     "Pranor Pulse PUBLISH order.created",
				Duration: 30,
				Offset:   75,
				Children: []WaterfallSpan{
					{

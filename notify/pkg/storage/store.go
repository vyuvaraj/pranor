...
func (s *PranorVaultTemplateStore) LoadTemplates() (map[string]map[string]string, error) {
	if s.Client == nil {
		return nil, errors.New("store client not initialized")
	}
	data, err := s.Client.Get("pranor-mail-templates", "templates.json")
	if err != nil {
		return nil, err
	}
	...
	log.Printf("[PERSISTENCE] Loaded %d templates from Pranor Vault", len(templates))
	return templates, nil
}

func (s *PranorVaultTemplateStore) SaveTemplates(templates map[string]map[string]string) error {
	if s.Client == nil {
	...
	return s.Client.Put("pranor-mail-templates", "templates.json", data)
}

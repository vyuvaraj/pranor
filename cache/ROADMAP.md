...
- [ ] **`pranor cache inspect` CLI** — CLI command to display per-namespace key counts, memory usage, hit/miss ratios, and top hot keys for developer debugging (DevOps)
- [ ] **Cache Warming Fixtures** — Load a named fixtures file (`pranor cache warm --fixture dev.yaml`) on service start for reproducible local development environments (DX)

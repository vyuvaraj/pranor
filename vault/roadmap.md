# Pranor Vault Implementation Roadmap

This document outlines the phase-wise development roadmap to transition Pranor Vault from the MVP to a fully distributed, enterprise-grade, S3-compatible object storage platform.
...
Bring Pranor Vault to high-scale production and Kubernetes environments.
- **Scalability**:
...
- **Kubernetes Operator**:
  - [x] Pranor Vault Kubernetes Operator for orchestration
  ...
  - [x] CSI (Container Storage Interface) Plugin support to expose Pranor Vault buckets as persistent storage volumes
...
|---|------|--------|-------------|--------|
| 8.1 | **Standardized `/healthz` and `/readyz` endpoints** | Small | Health and readiness probes for k8s liveness/readiness checks and Pranor Console aggregation. | [x] |
...
| 8.5 | **WebSocket push for cluster events** | Medium | Push real-time rebalance progress, node join/leave events, and replication lag updates to Pranor Console. | [x] |
| 8.6 | **Bucket event notifications** | Medium | Emit events (`s3:ObjectCreated`, `s3:ObjectRemoved`) to a configurable webhook or Pranor Pulse topic — enables event-driven architectures. | [x] |
...
These items take Pranor Vault from a production-ready S3-compatible engine to a **category-defining intelligent storage platform** — competing with MinIO, Ceph, and AWS S3 while offering unique AI-native capabilities.
...
| 9.5 | **S3 event notifications (CloudEvents)** | Medium | Emit CloudEvents-spec events on object lifecycle (`s3:ObjectCreated`, `s3:ObjectRemoved`, `s3:Replication`) to webhooks, Pranor Pulse topics, or NATS subjects. | [x] |
...
| 9.14 | **Federation (cross-cluster namespace)** | Large | Federate multiple Pranor Vault clusters under a single namespace. Global bucket names resolve to the owning cluster transparently — like DNS for objects. | [x] |

---

## Phase 10: Differentiating Factors — What No Other Storage Engine Offers (Strategic)

These create a **moat** around Pranor Vault — capabilities that MinIO, Ceph, AWS S3, and TurboBuffer cannot replicate without fundamental architecture changes.
...
| 10.6 | **AI-native object ingest pipeline** | Already Done | Text objects are automatically embedded and indexed on `PutObject` — no separate ETL pipeline, no external vector DB. Upload → searchable in one API call. | Auto-embedding on ingest. Competing systems require a separate pipeline (upload to S3 → trigger Lambda → call embedding API → write to Pinecone). Pranor Vault does it in-process. |
| 10.7 | **Ecosystem-integrated event streaming** | Medium | Object lifecycle events (`PutObject`, `DeleteObject`) publish to Pranor Pulse topics automatically. The storage engine IS the event source. No webhook relay, no S3 event notification config. | Native integration with Pranor Pulse. AWS needs S3→SNS→SQS or S3→EventBridge. MinIO needs webhook config. Pranor Vault publishes natively. |

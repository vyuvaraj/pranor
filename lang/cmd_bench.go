...
// It reads route declarations from the .pnr source, generates k6-style
// scenarios in-process, then fires HTTP requests and reports results.
//
// Usage: pranor bench <file.pnr> [--host <url>] [--rps <n>] [--duration <s>] [--output <json|text>]
func runBench() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: pranor bench <file.pnr> [--host <url>] [--rps <n>] [--duration <s>] [--output <json|text>]")
		fmt.Println("")
		fmt.Println("Examples:")
		fmt.Println("  pranor bench main.pnr")
		fmt.Println("  pranor bench main.pnr --host http://localhost:8080 --rps 100 --duration 30")
		fmt.Println("  pranor bench main.pnr --output json > bench-results.json")
		os.Exit(1)
	}
...
// extractBenchRoutes scans a .pnr file for route declarations.

...
func TestMetricsRegistry(t *testing.T) {
	...
	if !strings.Contains(body, `pranorVault_http_requests_total{method="GET",path="/test-bucket",status="200"} 2`) {
		t.Error("missing or incorrect GET request total metric")
	}
	if !strings.Contains(body, `pranorVault_http_requests_total{method="PUT",path="/test-bucket",status="201"} 1`) {
		t.Error("missing or incorrect PUT request total metric")
	}
	if !strings.Contains(body, "pranorVault_http_in_flight_requests 1") {
		t.Error("incorrect in-flight requests count")
	}
	if !strings.Contains(body, `pranorVault_http_request_duration_seconds_sum{method="GET",path="/test-bucket"} 0.400000`) {

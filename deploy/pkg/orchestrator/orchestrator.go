...
type Orchestrator struct {
	...
	workDir    string
	servPath   string // path to the 'pranor' compiler binary, if available
...
func NewOrchestrator(workDir string) (*Orchestrator, error) {
	...
	// Try to find the local 'pranor' binary in workspace if not on path
...
func (o *Orchestrator) buildAndRun(proc *ServiceProcess, srvDir, pnrFile string) {
	...
	// To make our orchestrator work with both .pnr compiler and Go, we will compile a helper Go script
	// if we don't have a working 'pranor' compiler, or if compilation fails.
...
func (o *Orchestrator) stopService(proc *ServiceProcess) {
	if proc.IsolationMode == "docker" {
		...
		if exec.Command("docker", "info").Run() == nil {
			exec.Command("docker", "stop", "pranor-"+proc.Name).Run()
			exec.Command("docker", "rm", "pranor-"+proc.Name).Run()
		}
	}
...
`, proc.Port, proc.Port)
	...
	if dockerAvailable {
		...
		go func() {
			logCmd := exec.Command("docker", "logs", "-f", "pranor-"+proc.Name)

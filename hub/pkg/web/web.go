package import (
	...
	"github.com/vyuvaraj/pranor/hub/pkg/resolution"
	"github.com/vyuvaraj/pranor/hub/pkg/signing"
)
...
func HandlePublish(w http.ResponseWriter, r *http.Request) {
	...
	// 1. Enforce JWT validation if PRANOR_JWT_SECRET is set
	if jwtSecret := os.Getenv("PRANOR_JWT_SECRET"); jwtSecret != "" {
	...
	// 2. Parse pranor.toml from uploaded tar.gz
	...
	if err != nil {
		...
		if name == "" {
			registry.WriteJSONError(w, r, "Missing 'name' parameter and no pranor.toml found", "ERR_MISSING_NAME_PARAMETER", http.StatusBadRequest)
			return
		}
...
func HandleGetPackage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		upstream := os.Getenv("PRANOR_UPSTREAM_REGISTRY")
		if upstream != "" {

...
You are Reviewer M2 for ServCache (SC.G3 & SC.G4).
...
Tasks:
1. Examine code in `packages/ServCache/pkg/bloom/bloom.go` and `packages/ServCache/pkg/tieredttl/policy.go`.
...
3. Run `go build ./...` and `go test -v -count=1 ./...` in `packages/ServCache`.

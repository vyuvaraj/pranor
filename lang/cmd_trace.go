...
func runTraceCmd() {
	if len(os.Args) < 3 {
		fmt.Println("Usage: pranor trace search [options] or pranor trace inspect <trace_id>")
		os.Exit(1)
	}
	...
	case "inspect":
		if len(os.Args) < 4 {
			fmt.Println("Usage: pranor trace inspect <trace_id>")
			os.Exit(1)
		}
...
func searchTraces() {
	...
	host := searchFlags.String("host", "http://localhost:8090", "Pranor Trace collector host")
	...
	authToken := os.Getenv("PRANOR_TRACE_AUTH_TOKEN")
	if authToken == "" {
	...
	if err != nil {
		fmt.Printf("Failed to connect to Pranor Trace collector: %v\n", err)
		os.Exit(1)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		fmt.Printf("Pranor Trace returned error status %d: %s\n", resp.StatusCode, string(body))
		os.Exit(1)
	}
...
func inspectTrace(traceID string) {
	...
	if envHost := os.Getenv("PRANOR_TRACE_URL"); envHost != "" {
		host = envHost
	}
...
func inspectTraceWithHost(host, traceID string) {
	...
	resp, err := client.Do(req)
	if err != nil {
		fmt.Printf("Failed to connect to Pranor Trace: %v\n", err)
		os.Exit(1)
	}
	defer resp.Body.Close()

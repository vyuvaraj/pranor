...
// MailSend forwards an email request to the Pranor Notify microservice.
func MailSend(to string, template string, data map[string]interface{}) error {
	endpoint := os.Getenv("PRANOR_MAIL_URL")
	if endpoint == "" {
	...
	token := os.Getenv("PRANOR_STORE_TOKEN")
	if token == "" {
...
// Notify forwards a generic channel notification (e.g. slack or sms) to the Pranor Notify microservice.

...
| 9.11 | **WASM transform marketplace** | Medium | Install community or private transforms via `pranor-pulse install <name>` resolving from Pranor Hub. Pre-built transforms: JSON→Protobuf, PII masking, enrichment. | [ ] |
...
| 9.13 | **Admin CLI (`pranor-pulse` binary)** | Medium | Terminal client supporting `topics list`, `topics create`, `publish`, `consume`, `offsets`, `transforms list/upload`, and cluster management. | [x] |

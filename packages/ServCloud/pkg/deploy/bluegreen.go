...
// BlueGreenManager orchestrates zero-downtime Blue/Green deployments and ServGate cutover.
...
// DeployNewVersion deploys to inactive slot, health checks it, and performs atomic cutover via ServGate.
func (bgm *BlueGreenManager) DeployNewVersion(ctx context.Context) (DeploymentTarget, error) {
	...
	// 2. Perform atomic ServGate route weight cutover
	if err := bgm.shiftGatewayTraffic(ctx, inactiveSlot, inactiveURL); err != nil {
		return "", fmt.Errorf("ServGate traffic cutover failed: %w", err)
	}
...
func (bgm *BlueGreenManager) shiftGatewayTraffic(ctx context.Context, newSlot DeploymentTarget, targetURL string) error {
	...
	if resp.StatusCode >= 300 {
		return fmt.Errorf("ServGate returned HTTP %d on cutover", resp.StatusCode)
	}
	return nil
}

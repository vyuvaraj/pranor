...
import (
	...
	"github.com/vyuvaraj/pranor/console/pkg/alerts"
)
...
var (
	...
	Runbooks = []RunbookAction{
		{ID: "rb-gate-restart", Name: "Restart Pranor Gate Instance", Description: "Drain connections and perform graceful restart of the Gateway process", Component: "github.com/vyuvaraj/pranor/gate", Command: "pranor gate restart --graceful"},
		{ID: "rb-gate-cache", Name: "Clear Pranor Gate Router Cache", Description: "Purge all compiled semantic cache entries in Gateway memory", Component: "github.com/vyuvaraj/pranor/gate", Command: "pranor gate cache purge"},
		{ID: "rb-store-heal", Name: "Rebalance Pranor Vault Storage Shards", Description: "Initiate P2P healing across active data shards and rebuild parity partitions", Component: "github.com/vyuvaraj/pranor/vault", Command: "pranor vault heal --shards=all"},
		{ID: "rb-queue-purge", Name: "Flush Dead Letter Queue (DLQ)", Description: "Clear stale or rejected messages in Pranor Pulse DLQ namespaces", Component: "github.com/vyuvaraj/pranor/pulse", Command: "pranor queue purge dlq"},
	}
...
func HandlePlaybooks(w http.ResponseWriter, r *http.Request) {
	...
	playbooks := []Playbook{
		...
		},
		{
			...
			Name:        "Pranor Gate High Latency Mitigation",
			Trigger:     "Pranor Gate p99 latency > 500ms for 30s",
			Steps:       3,
			LastRun:     "Never",
			AutoExecute: false,
		},
		{
			ID:          "pb-disk-pressure",
			Name:        "Pranor Vault Disk Pressure Relief",
			Trigger:     "Pranor Vault disk_usage_pct > 85%",
			Steps:       2,
			LastRun:     "12 days ago",
			AutoExecute: true,
		},

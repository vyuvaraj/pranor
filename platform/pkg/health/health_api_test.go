...
func TestUnifiedHealthAPI_RollupAndEndpoints(t *testing.T) {
	...
	api.ReportHealth(ComponentHealth{Name: "Pranor Gate", Healthy: true, Latency: 2 * time.Millisecond})
	api.ReportHealth(ComponentHealth{Name: "Pranor Pulse", Healthy: true, Latency: 3 * time.Millisecond})
	api.ReportHealth(ComponentHealth{Name: "Pranor Vault", Healthy: false, Message: "disk full"})
	...
	// /health endpoint - should return 503 because Pranor Vault is failing

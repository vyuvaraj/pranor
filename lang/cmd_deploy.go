...
func deployServ(pnrFile, target string) {
	...
	appName := strings.TrimSuffix(filepath.Base(pnrFile), ".pnr")
	port := "8080"
	...
	switch target {
	case "fly":
		generateFlyConfig(targetDir, appName, port, pnrFile)
	case "railway":
		generateRailwayConfig(targetDir, appName, pnrFile)
	case "render":
		generateRenderConfig(targetDir, appName, port, pnrFile)
	case "k8s", "kubernetes":
		generateK8sConfig(targetDir, appName, port, pnrFile)
	case "docker":
		dockerizeServ(pnrFile)
	default:
...
func generateFlyConfig(targetDir, appName, port, pnrFile string) {
	flyToml := fmt.Sprintf(`# Generated by: pranor deploy --target fly
app = "%s"
primary_region = "iad"
...
func generateRailwayConfig(targetDir, appName, pnrFile string) {
	railwayJSON := `{
...
func generateRenderConfig(targetDir, appName, port, pnrFile string) {
	renderYAML := fmt.Sprintf(`# Generated by: pranor deploy --target render
services:
  - type: web
    name: %s
    runtime: docker
...
func generateK8sConfig(targetDir, appName, port, pnrFile string) {
...
`, port)
	...
	} else if existingConfig, err := os.ReadFile(filepath.Join(targetDir, "pranor.toml")); err == nil {
		configContent = string(existingConfig)
	}

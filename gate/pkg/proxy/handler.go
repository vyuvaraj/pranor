...
func NewGatewayHandler(routes []Route, wasm *wasm.MiddlewareManager, authToken string) *GatewayHandler {
	...
	if redisURL := os.Getenv("PRANOR_GATE_LIMITS_REDIS_URL"); redisURL != "" {
		opt, err := redis.ParseURL(redisURL)
		if err == nil {
			rdb = redis.NewClient(opt)
			log.Printf("[Pranor Gate] Connected to Redis rate limiting backend at %s", redisURL)
		} else {
			log.Printf("[Pranor Gate] Failed to parse Redis URL %s: %v", redisURL, err)
		}
	}
...
func (h *GatewayHandler) isRateLimited(clientIP, routePrefix string, limit int) bool {
	...
	if h.redisClient != nil {
		...
		redisKey := "Pranor Gate:ratelimit:" + key
		...
		_, err := pipe.Exec(ctx)
		if err != nil {
			log.Printf("[Pranor Gate] Redis rate limiter communication error: %v. Falling back to local rate limiting.", err)
		} else {
			count := zCard.Val()
...
func (h *GatewayHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if tenantID != "" {
		...
		if exists {
			currentCluster := os.Getenv("PRANOR_CLUSTER")
			if currentCluster == "" {
			...
			currentRegion := os.Getenv("PRANOR_REGION")
			if currentRegion == "" {
	...
	if matchedRoute.SchemaName != "" && (r.Method == http.MethodPost || r.Method == http.MethodPut || r.Method == http.MethodPatch) {
		...
		if err == nil {
			...
			registryURL := os.Getenv("PRANOR_REGISTRY_URL")
			if registryURL == "" {
	...
	if strings.HasPrefix(selectedTarget, "pranor://") {
		...
		if rawDisc := os.Getenv("PRANOR_DISCOVERY"); rawDisc != "" {
			var manifest struct {
		...
		// Send request to Pranor Pulse API
...
func (h *GatewayHandler) serveOpenAPIDocs(w http.ResponseWriter, r *http.Request) {
	...
	openapi := map[string]interface{}{
		...
		"info": map[string]interface{}{
			"title":       "Pranor Gate API Gateway Discovery",
			"version":     "1.0.0",
...
<head>
  ...
  <title>Pranor Gate API Portal</title>
...
<body>
  <div class="header-panel">
    <div>
      <h1>⚡ Pranor Gate <span style="color: #818cf8;">Developer Portal</span><span class="badge">Docs</span></h1>
    </div>
  </div>

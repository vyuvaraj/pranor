...
var (
	...
	GateUrl     = flag.String("gate-url", "http://localhost:8080", "Pranor Gate base URL")
	StoreUrl    = flag.String("store-url", "http://localhost:8081", "Pranor Vault base URL")
	QueueUrl    = flag.String("queue-url", "http://localhost:8082", "Pranor Pulse base URL")
	TraceUrl    = flag.String("trace-url", "http://localhost:8090", "Pranor Trace base URL")
	TunnelUrl   = flag.String("tunnel-url", "http://localhost:8443", "Pranor Tunnel base URL")
	AuthUrl     = flag.String("auth-url", "http://localhost:8098", "Pranor Auth base URL")
	DbUrl       = flag.String("db-url", "http://localhost:8097", "ServDB base URL")
	MailUrl     = flag.String("mail-url", "http://localhost:8094", "Pranor Notify base URL")
	FlowUrl     = flag.String("flow-url", "http://localhost:8096", "Pranor Flow base URL")
	MeshUrl     = flag.String("mesh-url", "http://localhost:8089", "Pranor Mesh base URL")
	CronUrl     = flag.String("cron-url", "http://localhost:8087", "Pranor Chrono base URL")
	CacheUrl    = flag.String("cache-url", "http://localhost:8086", "Pranor Cache base URL")
	RegistryUrl = flag.String("registry-url", "http://localhost:8088", "Pranor Hub base URL")
	CloudUrl    = flag.String("cloud-url", "http://localhost:8085", "Pranor Deploy base URL")
	DocsUrl     = flag.String("docs-url", "http://localhost:8084", "ServDocs base URL")
	LockUrl     = flag.String("lock-url", "http://localhost:8089", "Pranor Lock base URL")
	SecretUrl   = flag.String("secret-url", "http://localhost:8091", "Pranor Secret base URL")
	AuthToken   = flag.String("auth-token", "gateway-secret-token", "Default API Auth token to use for downstream proxying")
	GateConfig  = flag.String("gate-config", "../Pranor Gate/config.json", "Path to Pranor Gate config.json")
...
func LoadDiscovery() ServDiscovery {
	d := ServDiscovery{
		...
		OTLPEndpoint: os.Getenv("PRANOR_OTLP_ENDPOINT"),
		JWTSecret:    os.Getenv("PRANOR_JWT_SECRET"),
	}

	raw := os.Getenv("PRANOR_DISCOVERY")
	if raw == "" {
		log.Println("[discovery] PRANOR_DISCOVERY not set — using CLI flags / defaults")
		return d
	}
	...
	if err := json.Unmarshal([]byte(raw), &manifest); err != nil {
		...
		if ferr != nil {
			log.Printf("[discovery] PRANOR_DISCOVERY is neither valid JSON nor a readable file: %v", ferr)
			return d
		}
		...
		log.Println("[discovery] Loaded from PRANOR_DISCOVERY env var (inline JSON)")
	}

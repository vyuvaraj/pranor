package import (
	...
	"github.com/vyuvaraj/pranor/console/pkg/config"
)
...
func HandleIngestLog(w http.ResponseWriter, r *http.Request) {
	...
	*LogBuffer = append(*LogBuffer, entry)
	...
	if strings.Contains(entry.Message, "[HIGH_LOAD]") {
		cloudURL := os.Getenv("PRANOR_CLOUD_URL")
		if cloudURL == "" {
	...
	if strings.Contains(entry.Message, "[SLOW_QUERY_ALERT]") || strings.Contains(entry.Message, "[DB_LATENCY_ALERT]") {
		go func(serviceName string) {
			...
			if os.Getenv("PRANOR_CONFIG_S3_BUCKET") != "" || os.Getenv("PRANOR_DISCOVERY") != "" {
...
func HandleSLO(w http.ResponseWriter, r *http.Request) {
	...
	slos := []SLOIndicator{
		{
			ServiceID:              "github.com/vyuvaraj/pranor/gate",
		...
		{
			ServiceID:              "github.com/vyuvaraj/pranor/vault",
		...
		{
			ServiceID:              "github.com/vyuvaraj/pranor/pulse",
		...
		{
			ServiceID:              "github.com/vyuvaraj/pranor/tunnel",
...
func HandleCostEstimation(w http.ResponseWriter, r *http.Request) {
	...
	gateStatus := CheckStatus("github.com/vyuvaraj/pranor/gate", *config.GateUrl)
	storeStatus := CheckStatus("github.com/vyuvaraj/pranor/vault", *config.StoreUrl)
	queueStatus := CheckStatus("github.com/vyuvaraj/pranor/pulse", *config.QueueUrl)
	...
	if gateRequests > 500000 {
		recommendations = append(recommendations, "Enable route-level caching on Pranor Gate to reduce CPU and baseline cost.")
	}
	if bucketsCount > 10 {
		recommendations = append(recommendations, "Consolidate unused S3 buckets to reduce storage index overhead.")
	}
	if storageGB > 100 {
		recommendations = append(recommendations, "Configure cold storage offloading policy in Pranor Pulse to migrate historical logs to standard compression.")
	}
	...
	response := map[string]any{
		...
		"breakdown": []map[string]any{
			...
			{"name": "Storage (Pranor Vault)", "value": storageCost, "color": "#10b981"},
			{"name": "Gateway (Pranor Gate)", "value": gateCost, "color": "#06b6d4"},
			{"name": "Queue (Pranor Pulse)", "value": queueCost, "color": "#f59e0b"},
		},
...
func HandleCapacityPlanning(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(CapacityResponse{
		...
		DiskUsagePct:     55.4,
		DaysToExhaust:    45,
		ForecastAnalysis: "Based on current storage consumption rate of 1.2 GB/day, disk capacity will be exhausted in approximately 45 days. Average CPU and Memory utilization remain stable. Recommending vertical scaling or archival rule configuration for Pranor Vault within 30 days to mitigate exhaustion risk.",
	})
}
	...
	events := []CorrelationEvent{
		{
			...
			Type:        "deployment",
			Title:       "Deployed Pranor Gate v1.4.2",
			Description: "Changelog: Optimize JSON marshal in proxy pipelines.",
			Source:      "github.com/vyuvaraj/pranor/deploy",
			Severity:    "info",
		},
		{
			Timestamp:   time.Now().Add(-8 * time.Minute),
			Type:        "alert",
			Title:       "High Latency Alert: Pranor Gate",
			Description: "p99 latency spiked to 312ms.",
			Source:      "github.com/vyuvaraj/pranor/console",
			Severity:    "warning",
		},
		{
...
func HandleNLQ(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		result := NLQResult{
			...
			ErrorCount:  17,
			TotalSpans:  284,
			Services:    []string{"github.com/vyuvaraj/pranor/gate", "github.com/vyuvaraj/pranor/vault", "ServDB"},
			Summary:     "Found 42 traces touching ServDB in the last hour. 17 contained errors (40%). Peak error rate at 14:03 UTC (p99 latency: 620ms). Most common error: 'context deadline exceeded' on SELECT queries. Likely correlated with Pranor Vault v1.4.2 deploy at 13:58 UTC.",
		}
...
func HandleMessageFlow(w http.ResponseWriter, r *http.Request) {
	...
	flow := MessageFlow{
		...
		Steps: []FlowStep{
			{Timestamp: now.Add(-10 * time.Second), Component: "github.com/vyuvaraj/pranor/gate", Action: "publish", Description: "Order checkout message received & published to broker", Status: "success"},
			{Timestamp: now.Add(-8 * time.Second), Component: "github.com/vyuvaraj/pranor/pulse", Action: "transform", Description: "Payload transformation schema mapping applied", Status: "success"},
			{Timestamp: now.Add(-6 * time.Second), Component: "github.com/vyuvaraj/pranor/vault", Action: "dlq", Description: "Database write deadlock; message routed to Dead Letter Queue (DLQ)", Status: "error"},
			{Timestamp: now.Add(-3 * time.Second), Component: "github.com/vyuvaraj/pranor/pulse", Action: "retry", Description: "DLQ retry policy triggered; re-processing message", Status: "success"},
			{Timestamp: now.Add(-1 * time.Second), Component: "github.com/vyuvaraj/pranor/flow", Action: "consume", Description: "Saga runner consumed and processed order", Status: "success"},
			{Timestamp: now, Component: "github.com/vyuvaraj/pranor/gate", Action: "ack", Description: "Consumer acknowledgment returned to broker", Status: "success"},
		},
	}
...
func HandleAIAssistantChat(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		"response": fmt.Sprintf("Pranor Console AI Assistant: Analyzed query '%s'. All components are operating within normal SLO error budget thresholds. Detected 0 trace anomalies.", req.Prompt),
		"mcp_tools_executed": []string{"Pranor Gate.get_routes", "Pranor Trace.explain_anomaly", "Pranor Pulse.get_consumer_lag"},

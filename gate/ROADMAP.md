...
This document outlines the planned evolutionary stages of **Pranor Gate** to evolve from a lightweight local proxy into a secure, distributed, WASM-programmable API Gateway.
...
- [x] **Pranor Gate → Pranor Pulse Webhook Bridge**: Register routes that publish directly to Pranor Pulse topics on incoming HTTP events. Connects gateway and broker tiers.
...
|---|------|--------|-------------|--------|
| 8.1 | **Standardized `/healthz` and `/readyz` endpoints** | Small | Expose health and readiness probes for Kubernetes liveness/readiness checks and Pranor Console health aggregation. | [x] |
...
| 8.6 | **CI/CD pipeline (GitHub Actions)** | Small | Automated build, test, and format checks on every PR. Currently missing — only Pranor has CI. | [x] |
| 8.7 | **WebSocket-based real-time metrics feed** | Medium | Push live connection counts, request rates, and error rates to Pranor Console via WebSocket (instead of polling). | [x] |
| 8.8 | **Config hot-reload without restart** | Medium | Watch `config.json` (or Pranor Vault bucket) for changes and apply route updates without restarting the gateway process. | [x] |

---

## Phase 9: Next-Level API Gateway (Proposed — Q4 2026+)

These items take Pranor Gate from a capable reverse proxy to a **category-defining programmable API platform** — competing with Kong, Envoy, and AWS API Gateway.

| # | Item | Effort | Description | Status |
|---|------|--------|-------------|--------|
| 9.1 | **OpenAPI auto-discovery** | Medium | Auto-generate an OpenAPI 3.1 spec from all registered routes — including request/response schemas inferred from Pranor type declarations. Serve at `/api/docs`. | [x] |
...
These create a **moat** around Pranor Gate — capabilities that Kong, Envoy, AWS API Gateway, and Traefik cannot replicate without fundamental architecture changes.
...
| 10.2 | **Compiler-aware route registration** | Small | ✅ Done — Dynamic route announcement via /api/v1/routes/register endpoint. Pranor services self-announce routes at startup. | Tight compiler→gateway integration. Impossible when gateway and language are separate products. |
...
| 10.7 | **Policy-as-code with `.policy` → WASM compilation** | Medium | Write human-readable access policies, compile to WASM, execute at wire speed. No Rego interpreter, no sidecar OPA process. Policy IS the binary. | Language-level compilation of policies. OPA/Rego is interpreted. Pranor Gate compiles policies to native speed. |
| 10.8 | **Pranor Pulse webhook bridge (event gateway)** | Small | A single route declaration can simultaneously proxy to a backend AND publish to a message queue topic. The gateway becomes an event sourcing entry point — not just a proxy. Already done. | Combined proxy + event sourcing in one hop. Other gateways need a separate webhook relay service. |
...
- [x] **GitOps Config Sync** — Git repository webhooks to automatically re-sync gateway routes (OPS.5)
- [x] **Auto TLS Let's Encrypt** — Integrated ACME client for automated certificate provisioning (OPS.6)

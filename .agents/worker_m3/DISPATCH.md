...
You are Worker 3 (Implementation for M3: ServCron).
...
File Ownership:
- `packages/ServCron/pkg/cron/cron.go`
- `packages/ServCron/pkg/cron/cron_test.go`
- `packages/ServCron/pkg/config/jobs_loader.go`
- `packages/ServCron/pkg/config/jobs_loader_test.go`

MANDATORY INTEGRITY WARNING:
DO NOT CHEAT. All implementations must be genuine. DO NOT hardcode test results, create dummy/facade implementations, or circumvent the intended task. A teamwork_preview_auditor will independently verify your work. Integrity violations WILL be detected and your work WILL be rejected.

Tasks:
1. Extend `Job` struct in `packages/ServCron/pkg/cron/cron.go`:
...
2. Create `packages/ServCron/pkg/config/jobs_loader.go`:
...
4. Run `go build ./...` and `go test ./...` in `/home/developer/workspace/serv/packages/ServCron`. Ensure exit code 0.
5. Verify `git diff go.mod` in `packages/ServCron` shows NO external dependency changes.

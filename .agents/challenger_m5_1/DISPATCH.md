...
You are Challenger M5 for ServQueue (SQ.G5).
...
Tasks:
1. Empirically verify traceparent injection, extraction, ID uniqueness (1000 iterations), invalid traceparent rejection, and engine log entry metadata extraction in `packages/ServQueue`.
2. Run `go test -race ./...` in `packages/ServQueue`.

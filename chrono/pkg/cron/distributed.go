...
// PranorLockElector implementation (EI.5)

func (sle *PranorLockElector) Start() {
	sle.wg.Add(1)
	go sle.electionLoop()
}

func (sle *PranorLockElector) Stop() {
	close(sle.stopChan)
	sle.wg.Wait()
...
func (sle *PranorLockElector) IsLeader() bool {
	sle.mu.Lock()
	defer sle.mu.Unlock()
	return sle.isLeader
}

func (sle *PranorLockElector) electionLoop() {
	defer sle.wg.Done()
...
func (sle *PranorLockElector) tryAcquireOrRenew() {
	sle.mu.Lock()
	defer sle.mu.Unlock()
	...
	if ok {
		if !sle.isLeader {
			log.Printf("[Pranor Lock] Node %s: Promoted to LEADER", sle.nodeID)
			sle.isLeader = true
		}
	} else {
		if sle.isLeader {
			log.Printf("[Pranor Lock] Node %s: Failed to renew leader lease. Stepping down.", sle.nodeID)
			sle.isLeader = false
		}
	}
}
...
func (sle *PranorLockElector) AcquireJobLock(jobID string, nextRun time.Time) bool {
	slotKey := fmt.Sprintf("Pranor Chrono:job:%s:run:%d", jobID, nextRun.Truncate(time.Minute).Unix())

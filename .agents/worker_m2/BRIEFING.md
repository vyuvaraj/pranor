...
## Mission
Implement M2 (ServCache roadmap features): SC.G3 (Probabilistic Bloom Filter) and SC.G4 (Tiered TTL Policy Engine) in `packages/ServCache`.
...
- Deliver code in `packages/ServCache/pkg/bloom` and `packages/ServCache/pkg/tieredttl`.
...
## Task Summary
- **What to build**:
  1. `Bloom` in `packages/ServCache/pkg/bloom/bloom.go` with FNV-1a double hashing, `NewBloom(capacity, fpRate)`, `Add(key)`, `MayContain(key)`.
  2. `TierPolicy` & `TieredCache` in `packages/ServCache/pkg/tieredttl/policy.go` with Hot (<=1s), Warm (<=5m), Cold (>5m) TTL classification, routing `Set` calls, tracking per-tier hit/miss counters via `Stats() TierStats`.
- **Success criteria**:
  - `go build ./...` and `go test -count=1 ./...` exit 0 in `packages/ServCache`.
...
## Artifact Index
- `/home/developer/workspace/serv/packages/ServCache/pkg/bloom/bloom.go` — Bloom filter implementation
- `/home/developer/workspace/serv/packages/ServCache/pkg/bloom/bloom_test.go` — Bloom filter unit tests
- `/home/developer/workspace/serv/packages/ServCache/pkg/tieredttl/policy.go` — Tiered TTL policy engine & cache wrapper
- `/home/developer/workspace/serv/packages/ServCache/pkg/tieredttl/policy_test.go` — Tiered TTL unit tests
...
## Change Tracker
- **Files modified**:
  - `packages/ServCache/pkg/bloom/bloom.go` — Created `Bloom` filter struct and math/FNV hashing functions.
  - `packages/ServCache/pkg/bloom/bloom_test.go` — Created 4 test functions for Bloom filter.
  - `packages/ServCache/pkg/tieredttl/policy.go` — Created `TierPolicy`, `TieredCache`, `TierStats`.
  - `packages/ServCache/pkg/tieredttl/policy_test.go` — Created 6 test functions for Tiered TTL policy engine.

...
// StoreClient handles communication with Pranor Vault (S3-compatible distributed storage).
...
func NewStoreClient() *StoreClient {
	endpoint := os.Getenv("PRANOR_STORE_URL")
	if endpoint == "" {
		endpoint = "http://localhost:8081"
	}
	authToken := os.Getenv("PRANOR_STORE_TOKEN")
	if authToken == "" {
...
// EnsureBucket creates a bucket in Pranor Vault if it does not already exist.
...
// Put writes object data to a bucket and key in Pranor Vault.
func (c *StoreClient) Put(bucket, key string, data []byte) error {
	if IsStandalone() {
		dir := filepath.Join(".Pranor Vault", bucket)
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated && resp.StatusCode != http.StatusNoContent {
		...
		return fmt.Errorf("failed to save to Pranor Vault (%d): %s", resp.StatusCode, string(body))
	}
	return nil
}

// Get reads object data from a bucket and key in Pranor Vault.
func (c *StoreClient) Get(bucket, key string) ([]byte, error) {
	if IsStandalone() {
		path := filepath.Join(".Pranor Vault", bucket, key)
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return nil, fmt.Errorf("failed to read from Pranor Vault (%d): %s", resp.StatusCode, string(body))
	}
	return io.ReadAll(resp.Body)
}

// SemanticSearch runs a vector similarity/semantic search query on a bucket in Pranor Vault.
...
// Delete deletes an object from a bucket and key in Pranor Vault.
func (c *StoreClient) Delete(bucket, key string) error {
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent && resp.StatusCode != http.StatusNotFound {
		body, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("failed to delete from Pranor Vault (%d): %s", resp.StatusCode, string(body))
	}
	return nil
}

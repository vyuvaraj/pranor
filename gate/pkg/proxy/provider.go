...
func (p *LocalFileProvider) Load() (*GatewayConfig, error) {
	...
	if secret := os.Getenv("PRANOR_JWT_SECRET"); secret != "" {
...
func NewS3ConfigProvider() *S3ConfigProvider {
	endpoint := os.Getenv("PRANOR_CONFIG_S3_ENDPOINT")
	bucket := os.Getenv("PRANOR_CONFIG_S3_BUCKET")
	key := os.Getenv("PRANOR_CONFIG_S3_KEY")
	accessKey := os.Getenv("PRANOR_CONFIG_S3_ACCESS_KEY")
	secretKey := os.Getenv("PRANOR_CONFIG_S3_SECRET_KEY")
	authToken := os.Getenv("PRANOR_CONFIG_S3_AUTH_TOKEN")
	...
	if endpoint == "" {
		endpoint = "http://localhost:8081" // fallback to Pranor Vault default
	}
	if bucket == "" {
		bucket = "pranor-config"
	}
	if key == "" {
...
func signConfig(cfg *GatewayConfig, secret []byte) error {
	...
	claims := map[string]interface{}{
		...
		"iss":         "github.com/vyuvaraj/pranor/console",
	}
...
func verifyConfigSignature(cfg *GatewayConfig, secret []byte) error {
	...
	if claims.Issuer != "github.com/vyuvaraj/pranor/console" {

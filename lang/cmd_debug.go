...
func debugServ(pnrFile string) {
	...
	fmt.Printf("[pranor debug] Compiling %s...\n", filepath.Base(absPath))
	buildDir, genGoFile, err := buildServDebug(absPath, program)
	if err != nil {
		fmt.Printf("[pranor debug] Build failed: %v\n", err)
		os.Exit(1)
	}
	fmt.Printf("[pranor debug] Build dir: %s\n", buildDir)

	// 2. Parse the generated Go file to build the source map.
	sm, err := dap.ParseSourceMap(genGoFile, absPath)
	if err != nil {
		fmt.Printf("[pranor debug] Source map: %v\n", err)
		os.Exit(1)
	}
	fmt.Printf("[pranor debug] Source map: %d entries\n", sm.Len())

	// 3. Find a free local TCP port for dlv.
	port, err := freePort()
	if err != nil {
		fmt.Printf("[pranor debug] No free port: %v\n", err)
		os.Exit(1)
	}
	...
	if _, err := exec.LookPath("dlv"); err != nil {
		fmt.Println("[pranor debug] 'dlv' not found in PATH.")
	...
	fmt.Printf("[pranor debug] Starting Delve DAP server on %s...\n", dlvAddr)
	dlvCmd := exec.Command(
		"dlv", "dap",
		"--listen="+dlvAddr,
		"--headless",
	...
	if err := dlvCmd.Start(); err != nil {
		fmt.Printf("[pranor debug] Failed to start dlv: %v\n", err)
		os.Exit(1)
	}
	defer func() {
	...
	if err := waitForPort(dlvAddr, 5*time.Second); err != nil {
		fmt.Printf("[pranor debug] dlv not ready: %v\n", err)
		os.Exit(1)
	}
	fmt.Printf("[pranor debug] Delve ready. Starting DAP proxy on stdio.\n")

	// 7. Run the DAP proxy on stdin/stdout.
	proxy := dap.NewProxy(sm, dlvAddr)
	if err := proxy.Run(os.Stdin, os.Stdout); err != nil {
		fmt.Fprintf(os.Stderr, "[pranor debug] Proxy exited: %v\n", err)
	}
}

// buildServDebug compiles the .pnr file with debug-friendly Go build flags:

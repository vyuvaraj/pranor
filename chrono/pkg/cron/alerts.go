...
func buildSlackPayload(payload AlertPayload) ([]byte, error) {
	color := "#danger"
	title := fmt.Sprintf("🚨 Pranor Chrono Alert: Job %s Failed", payload.JobID)
	if payload.LastOutcome == "success" {
		color = "#good"
		title = fmt.Sprintf("✅ Pranor Chrono Recovered: Job %s Succeeded", payload.JobID)
	}
...
func buildPagerDutyPayload(routingKey string, payload AlertPayload) ([]byte, error) {
	...
	pdMsg := map[string]interface{}{
		...
		"event_action": action,
		"dedup_key":    "Pranor Chrono-" + payload.JobID,
		"payload": map[string]interface{}{
			"summary":   fmt.Sprintf("Pranor Chrono Job %s: %s (Failures: %d)", payload.JobID, payload.LastOutcome, payload.FailureCount),
			"source":    "Pranor Chrono",
			"severity":  "critical",
...
func FormatSlackAlertHelper(jobID, targetURL, errStr string) string {
	return fmt.Sprintf("*Pranor Chrono Failure*: `%s` (%s) - Error: %s", jobID, targetURL, strings.TrimSpace(errStr))
}

...
// ServConfig is the schema for .pranor/config.yaml
...
// runConfigPropagate reads .pranor/config.yaml and pushes config to all
// running Pranor services via their /api/config endpoint.
// Usage: pranor config propagate [--file <path>] [--dry-run] [--services <list>]
func runConfigPropagate() {
	configPath := filepath.Join(".pranor", "config.yaml")
	dryRun := false
	servicesFilter := ""
	...
	if err != nil {
		...
		fmt.Println("Run 'pranor config init' to create a starter .pranor/config.yaml")
		os.Exit(1)
	}
...
// runConfigInit creates a starter .pranor/config.yaml
func runConfigInit() {
	dir := ".pranor"
...
`
	...
	fmt.Println("    pranor config propagate")
	fmt.Println("  Preview without applying:")
	fmt.Println("    pranor config propagate --dry-run")
}

// runConfigCmd is the top-level dispatcher for 'pranor config'
func runConfigCmd() {
	subcmd := ""
	if len(os.Args) >= 3 {
		subcmd = os.Args[2]
	...
	default:
		...
		fmt.Println("  pranor config propagate [--dry-run]            Push config to all running services")
		fmt.Println("  pranor config propagate --services gate,cache  Push to specific services only")
	}
}

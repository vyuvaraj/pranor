...
func TestParserStatements(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: `extern fn fetch_user(id) from "go:github.com/vyuvaraj/pranor/lang/runtime:FetchUser"`,
			checkNode: func(t *testing.T, program *Program) {
				if len(program.Statements) != 1 {
					t.Fatalf("expected 1 statement, got %d", len(program.Statements))
				}
				...
				if extStmt.Name != "fetch_user" || extStmt.Source != "go:github.com/vyuvaraj/pranor/lang/runtime:FetchUser" {
...
func TestEcosystemASTParsingBattery(t *testing.T) {
	...
	inputs := []string{
		...
		"mock db.query { return 1 }", "import \"stdlib/s3.pnr\"", "import { get, put } from \"stdlib/s3.pnr\"",

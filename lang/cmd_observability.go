...
// ObservabilityConfig is the schema for .pranor/observability.yaml
...
// runObservabilityApply reads .pranor/observability.yaml and provisions
// alert rules, SLOs, and dashboards into Pranor Console.
// Usage: pranor observability apply [--console <url>] [--dry-run]
func runObservabilityApply() {
	configPath := filepath.Join(".pranor", "observability.yaml")
	...
	if err != nil {
		...
		fmt.Println("Create .pranor/observability.yaml first. Run: pranor observability init")
		os.Exit(1)
	}
...
// runObservabilityInit creates a starter .pranor/observability.yaml
...
# SLO definitions — tracked by Pranor Console error budget panel
slos:
  - service: Pranor Gate
...
# Alert rules — provisioned into Pranor Console on deploy
alerts:
...
`
	...
	fmt.Println("    pranor observability apply")
	fmt.Println("  Preview without applying:")
	fmt.Println("    pranor observability apply --dry-run")
}

// runObservabilityCmd is the top-level dispatcher for 'pranor observability'
func runObservabilityCmd() {
	...
	default:
		fmt.Println("Usage:")
		fmt.Println("  pranor observability init                   Create starter .pranor/observability.yaml")
		fmt.Println("  pranor observability apply [--dry-run]      Provision alert rules, SLOs, dashboards")
		fmt.Println("  pranor observability apply --console <url>  Target a specific Pranor Console instance")
	}
}

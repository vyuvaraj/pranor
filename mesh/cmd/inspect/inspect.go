// Package main implements the `Pranor Mesh inspect` CLI subcommand.
// It queries the Pranor Mesh registry /api/topology endpoint and renders a
...
func main() {
	registry := flag.String("registry", "http://localhost:8089", "Pranor Mesh registry URL")
...
func render(registryURL, serviceFilter string) error {
	...
	now := time.Now()
	fmt.Printf("%s%s Pranor Mesh Topology Inspector%s — %s\n",

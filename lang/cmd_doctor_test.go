...
func TestDoctorCommandOffline(t *testing.T) {
	...
	os.Setenv("PRANOR_DISCOVERY", string(discBytes))
	defer os.Unsetenv("PRANOR_DISCOVERY")

	// Set invalid trace endpoint to trigger a warning/error in telemetry pipeline
	os.Setenv("PRANOR_OTLP_ENDPOINT", "http://127.0.0.1:59993")
	defer os.Unsetenv("PRANOR_OTLP_ENDPOINT")
...
func TestDoctorCommandOnline(t *testing.T) {
	...
	os.Setenv("PRANOR_OTLP_ENDPOINT", mockCollector.URL)
	defer os.Unsetenv("PRANOR_OTLP_ENDPOINT")

...
func HandleRebalance(w http.ResponseWriter, r *http.Request) {
	...
	body := strings.NewReader(`{"source_node":{"node_id":"github.com/vyuvaraj/pranor/console","address":"localhost:8083","status":"online"},"peers":{}}`)
	...
	resp, err := client.Do(req)
	if err != nil {
		http.Error(w, "Pranor Vault unreachable: "+err.Error(), http.StatusBadGateway)
		return
	}
	defer resp.Body.Close()
	user := r.Header.Get("X-Console-User")
	...
	log.Printf("Rebalance gossip round triggered, Pranor Vault responded: %d", resp.StatusCode)
	w.Header().Set("Content-Type", "application/json")
...
func HandleDiagnosticExec(w http.ResponseWriter, r *http.Request) {
	...
	case "ps aux":
		output = fmt.Sprintf("USER       PID %%CPU %%MEM    VSZ    RSS TTY      STAT START   TIME COMMAND\n" +
			...
			"operator    42  1.4  3.2 245760 65536 ?        Sl   10:05   0:15 /usr/local/bin/pranor %s\n" +
	...
	case "pranor status":
		output = fmt.Sprintf("Pranor Component: %s\n" +
			"Status: ACTIVE\n" +
			"Uptime: 2h 45m 12s\n" +
			"Version: v1.4.2-stable\n" +
			"Config Load: /etc/pranor/config.json (OK)\n" +
	...
	default:
		if strings.HasPrefix(cmd, "ping ") {
			...
			output = fmt.Sprintf("bash: %s: command not found\nAvailable diagnostic tools: ps aux, free -m, df -h, pranor status, ping [target]", req.Command)
			status = "error"
		}
	}

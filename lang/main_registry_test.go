...
func TestPackageRegistryIntegration(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "PRANOR_registry_test")
	if err != nil {
...
`
	if err := os.WriteFile(filepath.Join(srcPkgDir, "index.pnr"), []byte(pkgFileContent), 0644); err != nil {
	...
	for {
		...
		if hdr.Name == "index.pnr" {
			foundFile = true
		}
	}
	gr.Close()
	if !foundFile {
		t.Errorf("registry tarball is missing index.pnr")
	}
	...
	// Verify extraction
	installedFile := filepath.Join("packages", "mypkg", "index.pnr")
...
`
	importerPath := filepath.Join(tmpDir, "importer.pnr")
	if err := os.WriteFile(importerPath, []byte(importerContent), 0644); err != nil {
		t.Fatalf("failed to write importer.pnr: %v", err)
	}
